...
const collectDecksData = async (
	data: SyncPending
): Promise<Partial<SyncData>> => {
	const updateDecks: SyncDataDeck[] | undefined = (
		await getMany(
			uniqueSyncData(data.updateDecks).map(({ key }: SyncPendingDeck) => key)
		)
	).map((deck: Deck) => ({
		deckId: deck.id,
		deck,
	...
	const deleteDecks: SyncDataDeck[] | undefined = uniqueSyncData(
		data.deleteDecks
	).map(({ deckId }: SyncPendingDeck) => ({ deckId }));

	const updateSlides: SyncDataSlide[] | undefined = await Promise.all(
		uniqueSyncData(data.updateSlides).map((slide: SyncPendingSlide) =>
			getSlide(slide)
		)
	);

	const deleteSlides: SyncDataSlide[] | undefined = uniqueSyncData(
		data.deleteSlides
	).map(({ deckId, slideId }: SyncPendingSlide) => ({
		deckId,
		slideId,
	}));
	...
	return {
		updateDecks: deleteDecks
			? updateDecks?.filter(
					({ deckId }: SyncDataDeck) =>
						!deleteDecks.find(
							({ deckId: deleteDeckId }: SyncDataDeck) =>
								deleteDeckId === deckId
						)
			  )
			: updateDecks,
		...
		updateSlides: deleteSlides
			? updateSlides?.filter(
					({ slideId }: SyncDataSlide) =>
						!deleteSlides.find(
							({ slideId: deleteSlideId }: SyncDataSlide) =>
...
const collectDocsData = async (
	data: SyncPending
): Promise<Partial<SyncData>> => {
	const updateDocs: SyncDataDoc[] | undefined = (
		await getMany(
			uniqueSyncData(data.updateDocs).map(({ key }: SyncPendingDoc) => key)
		)
	).map((doc: Doc) => ({
		docId: doc.id,
		doc,
	...
	const deleteDocs: SyncDataDoc[] | undefined = uniqueSyncData(
		data.deleteDocs
	).map(({ docId }: SyncPendingDoc) => ({ docId }));

	const updateParagraphs: SyncDataParagraph[] | undefined = await Promise.all(
		uniqueSyncData(data.updateParagraphs).map(
			(paragraph: SyncPendingParagraph) => getParagraph(paragraph)
		)
	);
	...
	return {
		updateDocs: deleteDocs
			? updateDocs?.filter(
					({ docId }: SyncDataDoc) =>
						!deleteDocs.find(
							({ docId: deleteDocId }: SyncDataDoc) => deleteDocId === docId
						)
			  )
			: updateDocs,
		deleteDocs,
...
const getSlide = async ({
	...
	slideId,
	key,
}: SyncPendingSlide): Promise<SyncDataSlide> => {
...
const uniqueSyncData = (
	data: SyncPendingData[] | undefined
): SyncPendingData[] => {
	return (data || []).reduce(
		(acc: SyncPendingData[], curr: SyncPendingData) => {
			const index: number = acc.findIndex(
				({ key }: SyncPendingData) => key === curr.key
			);
