...
export class ImageHistoryService {
	...
	async push(image: UnsplashPhoto | TenorGif | StorageFile | Waves) {
		...
		let images: (UnsplashPhoto | TenorGif | StorageFile | Waves)[] =
			await this.get();
		...
		const exist: boolean = image.hasOwnProperty('viewBox')
			? false
			: this.exists(images, image);
	...
	private exists(
		...
		const index: number = images
			.filter(
				(filteredPhoto: UnsplashPhoto | TenorGif | StorageFile | Waves) =>
					!filteredPhoto.hasOwnProperty('viewBox')
			)
			.findIndex(
				(filteredPhoto: UnsplashPhoto | TenorGif | StorageFile | Waves) => {
					if (filteredPhoto.hasOwnProperty('fullPath')) {
						return (
							(filteredPhoto as StorageFile).fullPath ===
							(image as StorageFile).fullPath
						);
					} else {
						return (
							(filteredPhoto as UnsplashPhoto | TenorGif).id ===
							(image as UnsplashPhoto | TenorGif).id
						);
					}
				}
			);
