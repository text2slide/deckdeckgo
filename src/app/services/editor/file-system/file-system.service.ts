...
import {
	...
	importEditorData,
	importEditorSync,
} from '../../../utils/editor/import.utils';
import {
...
import { AppDecks } from '../../../pages/core/app-decks/app-decks';
...
export class FileSystemService {
	...
	async exportData() {
		...
		const localSlidesImages: UserAsset[] = await getSlidesLocalImages({
			deck: editorStore.state.deck,
		});
		const onlineSlidesImages: UserAsset[] = await getSlidesOnlineImages({
			deck: editorStore.state.deck,
		});

		const localSlidesCharts: UserAsset[] = await getSlidesLocalCharts({
			deck: editorStore.state.deck,
		});
		const onlineSlidesCharts: UserAsset[] = await getSlidesOnlineCharts({
			deck: editorStore.state.deck,
		});

		const deckBackground: UserAsset | undefined =
			await getDeckBackgroundImage();

		const localParagraphsImages: UserAsset[] = await getParagraphsLocalImages({
			doc: editorStore.state.doc,
		});
		const onlineParagraphsImages: UserAsset[] = await getParagraphsOnlineImages(
			{ doc: editorStore.state.doc }
		);
		...
		await this.save({
			filename:
				editorStore.state.deck?.data?.name || editorStore.state.doc?.data?.name,
			blob,
		});
	}
	...
	private async getSlides(): Promise<Slide[] | undefined> {
		...
		try {
			const keys: string[] = deck.data.slides.map(
				(slideId: string) => `/decks/${deck.id}/slides/${slideId}`
			);
	...
	private async getParagraphs(): Promise<Paragraph[] | undefined> {
		...
		try {
			const keys: string[] = doc.data.paragraphs.map(
				(paragraphId: string) => `/docs/${doc.id}/paragraphs/${paragraphId}`
			);
	...
	private async unzip(file: File): Promise<{
		...
		this.listZipAssets({
			content,
			zippedAssets,
			subPath: '/assets/local/images/',
		});
		this.listZipAssets({
			content,
			zippedAssets,
			subPath: '/assets/local/data/',
		});
		...
		if (!offlineStore.state.online) {
			this.listZipAssets({
				content,
				zippedAssets,
				subPath: '/assets/online/images/',
			});
			this.listZipAssets({
				content,
				zippedAssets,
				subPath: '/assets/online/data/',
			});
		}
	...
	private listZipAssets({
		...
		content
			.folder(subPath)
			.forEach((filename: string, file: JSZip.JSZipObject) =>
				zippedAssets.push({
	...
	private async parseImportData(content: JSZip): Promise<FileImportData> {
		...
		if (!offlineStore.state.online) {
			const assetsContent: string | null = await content
				.file('assets.json')
				?.async('text');
			const assets: UserAsset[] = assetsContent
				? JSON.parse(assetsContent)
				: [];
	 ...
	 */
	private resetImportDataIds({
		deck,
		slides,
		doc,
		paragraphs,
	}: FileImportData): { data: ImportData } {
		const id: string = uuid();
		const now: Date = new Date();

		const newSlides: Slide[] | undefined = slides?.map(
			(slide: Partial<Slide>) => ({
				data: {
					...slide.data,
					updated_at: now,
		...
		const newParagraphs: Paragraph[] | undefined = paragraphs?.map(
		...
		return {
			data: {
				id,
				...(newDeck && {
					deck: this.cleanDeck({ deck: newDeck, cleanMeta: true }),
				}),
