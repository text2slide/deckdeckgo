...
export const initAuthProvider = async () => {
	...
	const { initAuth }: { initAuth: InitAuth } = await cloudProvider<{
		initAuth: InitAuth;
	}>();
...
export const signOut = async () => {
	...
	const { signOut }: { signOut: SignOut } = await cloudProvider<{
		signOut: SignOut;
	}>();
...
export const deleteAuth = async () => {
	...
	const { deleteAuth }: { deleteAuth: DeleteAuth } = await cloudProvider<{
...
const firebaseApiConfig = (): Record<string, string | boolean> => {
	...
	const { mock }: EnvironmentAppConfig =
		EnvironmentConfigService.getInstance().get('app');
