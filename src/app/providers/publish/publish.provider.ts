...
export const publishUrl = async (meta: Meta | undefined): Promise<string> => {
	...
	if (cloud() && published) {
		const { publishUrl }: { publishUrl: PublishUrl } = await cloudProvider<{
...
const publishDoc = async (inputs: PublishInputs): Promise<void> => {
	...
	const { docPublish }: { docPublish: DocPublish } = await cloudProvider<{
...
const publishDeck = async (inputs: PublishInputs): Promise<void> => {
	...
	const { deckPublish }: { deckPublish: DeckPublish } = await cloudProvider<{
		deckPublish: DeckPublish;
	}>();

	const firebaseConfig: Record<string, string> =
		EnvironmentConfigService.getInstance().get('firebase');
...
const updateMeta = ({
	...
	if (userStore.state.user?.data?.name) {
		...
		if (userStore.state.user.data.photo_url) {
			(updateMeta.author as Author).photo_url =
				userStore.state.user.data.photo_url;
		}

		if (userStore.state.user.data.social) {
			(updateMeta.author as Author).social = Object.keys(
				userStore.state.user.data.social
			).reduce((acc: UserSocial, key: string) => {
				acc[key] =
					userStore.state.user.data.social[key] !== null &&
					userStore.state.user.data.social[key] !== undefined
