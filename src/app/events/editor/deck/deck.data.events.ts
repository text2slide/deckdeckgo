...
import { AppCreateSlide } from '../../../popovers/editor/app-create-slide/app-create-slide';
...
export class DeckDataEvents {
	...
	private readonly debounceUpdateSlide_noDelay: (slide: HTMLElement) => void;
	...
	constructor() {
		...
		this.debounceUpdateSlide_noDelay = async (element: HTMLElement) => {
			await this.updateSlide(element);
	...
	async init(el: HTMLElement) {
		...
		this.mainRef.addEventListener(
			'markdownDidChange',
			this.onCustomEventChange
		);
		this.mainRef.addEventListener(
			'wordCloudDidChange',
			this.onCustomEventChange
		...
		this.mainRef.addEventListener('drrTextDidChange', this.onCustomEventChange);

		this.mainRef.addEventListener('deckDidLoad', this.onDeckDidLoad, {
			once: true,
		});
		...
		document.addEventListener(
			'slideDidChangeNoDelay',
			this.onSlideChangeNoDelay
		);
		document.addEventListener('slideDelete', this.onSlideDelete);
		document.addEventListener('slidesDelete', this.onSlidesDelete);
		...
		document.addEventListener('customAction', this.onCustomEventChange);
	}
	...
	destroy() {
		...
		this.mainRef.removeEventListener(
			'markdownDidChange',
			this.onCustomEventChange
		);
		this.mainRef.removeEventListener(
		...
		document.removeEventListener('slidesDelete', this.onSlidesDelete);
	...
	private onSlideChangeNoDelay = async ($event: CustomEvent) => {
		if (!$event || !$event.detail) {
			return;
		}

		this.debounceUpdateSlide_noDelay($event.detail);
	};
	...
	private onCustomEventChange = async ($event: CustomEvent) => {
		...
		if (
			SlotUtils.isNodeReveal(parent) ||
			SlotUtils.isNodeDragDropResize(parent)
		) {
	...
	private onBackspaceClick = ($event: KeyboardEvent) => {
		...
		if (
			...
			if (spanEl.style.fontSize === '1.4em') {
				spanEl.style.fontSize = 'inherit';
			}
		}
	};
	...
	private onInputChange = async ($event: InputEvent) => {
		...
		/* *NOTE* TEMP SOLUTION: if element contains child which is a span and fontsize is 1.4em, change it to inherit */
		element.addEventListener('keyup', this.onBackspaceClick);
		...
		if (
			...
			SlotUtils.isNodeTitle(element)
		) {
			this.debounceUpdateDeckTitle(element.textContent);
		}
		// element.removeEventListener('keydown', this.onBackspaceClick);
	};
	...
	private onSlidesDelete = async ($event: CustomEvent) => {
		if (!$event || !$event.detail) {
			return;
		}

		$event.detail.forEach(async (slide) => {
			await this.deleteSlide(slide);
		});
	};
	...
	private createSlide(slide: HTMLElement): Promise<void> {
		return new Promise<void>(async (resolve) => {
			try {
				...
				slide.setAttribute('editable', 'true');
				...
				const persistedSlide: Slide = await this.postSlide(
					editorStore.state.deck,
					slide
				);

				await this.updateDeckSlideList(
					editorStore.state.deck,
					persistedSlide,
					slide
				);
...
return new Promise<Slide>(async (resolve) => {
			const slideData: SlideData = await this.initSlideData(slide);

			const content: string | null = this.cleanSlide(slide);
			if (content?.length > 0) {
				slideData.content = content;
			}
			// *NOTE* if there are default content, store them
			if (slide.childNodes?.length > 0) {
				slideData.content = slide.innerHTML;
			}

			const attributes: SlideAttributes = await this.getSlideAttributes(
				slide,
				false
			);
			...
			const persistedSlide: Slide = await this.slideOfflineProvider.create(
				deck.id,
				slideData
			);
	...
	private updateDeckSlideList(
		...
		return new Promise<void>(async (resolve, reject) => {
			try {
				...
				deck.data.slides = [
					...deck.data.slides.slice(0, slideIndex),
					slide.id,
					...deck.data.slides.slice(slideIndex),
				];
	...
	private updateDeck(deck: HTMLElement): Promise<void> {
		return new Promise<void>(async (resolve) => {
			try {
				...
				const attributes: DeckAttributes = await this.getDeckAttributes(
					deck,
					true,
					currentDeck
				);
				...
				const [background, header, footer]: (string | null)[] = [
					'background',
					'header',
					'footer',
				].map((slotName: 'background' | 'header' | 'footer') =>
					this.getDeckSlot(deck, slotName)
				);
				...
				// @ts-ignore
				currentDeck.data.header =
					header && header !== undefined && header !== '' ? header : null;

				// @ts-ignore
				currentDeck.data.footer =
					footer && footer !== undefined && footer !== '' ? footer : null;

				const updatedDeck: Deck = await this.deckOfflineProvider.update(
					currentDeck
				);
		...
		return new Promise<void>(async (resolve) => {
			try {
				...
				const slideUpdate: Slide = {
					...
					data: await this.initSlideData(slide),
				};

				const content: string | null = this.cleanSlide(slide);
				if (content?.length > 0) {
					slideUpdate.data.content = content;
				} else if (slide.childNodes?.length > 0) {
					slideUpdate.data.content = slide.innerHTML;
				} else {
					// @ts-ignore
				...
				if (editorStore.state.deck) {
					await this.slideOfflineProvider.update(
			...
			try {
				...
				if (currentDeck && currentDeck.data) {
					...
					if (slide && slide.data) {
						...
						if (
							currentDeck.data.slides &&
							currentDeck.data.slides.indexOf(slideId) > -1
						) {
							currentDeck.data.slides.splice(
								currentDeck.data.slides.indexOf(slideId),
								1
							);
	...
	private async getSlideAttributes(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
	...
	private async getSlideUserAttributes(
		slide: HTMLElement
	): Promise<SlideAttributes> {
		return Array.from(slide.attributes)
			?.filter((a: Attr) => {
				return !/(?:contenteditable|editable|spellcheck|highlighted|custom-loader|class|slide_id|type)/.test(
					a.name
				);
			})
	...
	private async getSlideDefaultAttributes(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
		...
		if (slide.getAttribute('type')) {
			const type = slide.getAttribute('type');
			if (Object.keys(SlideChartType).includes(type.toUpperCase())) {
				attributes.type = slide.getAttribute('type') as SlideChartType;
			}
		}
		...
		const qrCodeAttributes: SlideAttributes =
			await this.getSlideAttributesQRCode(slide, cleanFields);
		const chartAttributes: SlideAttributes = await this.getSlideAttributesChart(
			slide,
			cleanFields
		);
		const splitAttributes: SlideAttributes = await this.getSlideAttributesSplit(
			slide,
			cleanFields
		);
		const authorAttributes: SlideAttributes =
			await this.getSlideAttributesAuthor(slide, cleanFields);
		const playgroundAttributes: SlideAttributes =
			await this.getSlideAttributesPlayground(slide, cleanFields);
	...
	private async getSlideAttributesSplit(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
		if (
			!slide ||
			!slide.nodeName ||
			slide.nodeName.toLowerCase() !== 'deckgo-slide-split'
		) {
			return {};
		}
		...
		if (
			slide.hasAttribute('type') &&
			(slide.getAttribute('type') as SlideSplitType) === SlideSplitType.DEMO
		) {
	...
	private async getSlideAttributesAuthor(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
		if (
			!slide ||
			!slide.nodeName ||
			slide.nodeName.toLowerCase() !== 'deckgo-slide-author'
		) {
			return {};
		}
		...
		if (
			slide.hasAttribute('img-mode') &&
			slide.getAttribute('img-mode') !== 'cover'
		) {
	...
	private async getSlideAttributesPlayground(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
		if (
			!slide ||
			!slide.nodeName ||
			slide.nodeName.toLowerCase() !== 'deckgo-slide-playground'
		) {
			return {};
		}
	...
	private async getSlideAttributesQRCode(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
		if (
			!slide ||
			!slide.nodeName ||
			slide.nodeName.toLowerCase() !== 'deckgo-slide-qrcode'
		) {
			return {};
		}
	...
	private async getSlideAttributesChart(
		slide: HTMLElement,
		cleanFields: boolean
	): Promise<SlideAttributes> {
		if (
			!slide ||
			!slide.nodeName ||
			slide.nodeName.toLowerCase() !== 'deckgo-slide-chart-custom'
		) {
			return {};
		}
		...
		if (slide.hasAttribute('type')) {
			attributes.type =
				SlideChartType[slide.getAttribute('type').toUpperCase()];
		}
		...
		if (slide.hasAttribute('y-axis-domain')) {
			attributes.yAxisDomain = slide.getAttribute(
				'y-axis-domain'
			) as SlideAttributesYAxisDomain;
		} else if (cleanFields) {
			// @ts-ignore
	...
	private async getDeckAttributes(
		deck: HTMLElement,
		updateDeck: boolean,
		currentDeck: Deck | null = null
	): Promise<DeckAttributes> {
		...
		if (
			deck.hasAttribute('animation') &&
			deck.getAttribute('animation') !== 'slide'
		) {
			attributes.animation = deck.getAttribute('animation') as
				| 'slide'
				| 'fade'
				| 'none';
		} else if (updateDeck) {
		...
		if (
			deck.hasAttribute('direction') &&
			deck.getAttribute('direction') !== 'horizontal'
		) {
			attributes.direction = deck.getAttribute('direction') as
				| 'horizontal'
				| 'vertical'
				| 'papyrus';
		} else if (updateDeck) {
		...
		if (
			deck.hasAttribute('direction-mobile') &&
			deck.getAttribute('direction-mobile') !== 'papyrus'
		) {
			attributes.directionMobile = deck.getAttribute('direction-mobile') as
				| 'horizontal'
				| 'vertical'
				| 'papyrus';
		} else if (updateDeck) {
	...
	private getDeckSlot(
		deck: HTMLElement,
		slotName: 'background' | 'header' | 'footer'
	): string | null {
		const slotElement: HTMLElement = deck.querySelector(
			slotName === 'background'
				? `:scope > [slot='${slotName}']`
				: `:scope [slot='${slotName}']`
		);
...
return result;
	...
	private getSlideTemplate(slide: HTMLElement): SlideTemplate | undefined {
		const templateKey: string | undefined = Object.keys(SlideTemplate).find(
			(key: string) => {
	...
	private async initSlideData(slide: HTMLElement): Promise<SlideData> {
		const template: SlideTemplate | undefined = this.getSlideTemplate(slide);

		return {
			template: template || slide.nodeName?.toLowerCase(),
			attributes: await this.getSlideAttributes(slide, true),
			...(!template && { scope: SlideUtils.slideScope(slide) }),
		};
	}

	async initSlideSize() {
		const deck: HTMLDeckgoDeckElement =
			this.mainRef.querySelector('deckgo-deck');
		...
		return new Promise<void>(async (resolve, reject) => {
			try {
				...
				if (
					...
					currentDeck.data &&
					currentDeck.data.slides &&
					detail.to < currentDeck.data.slides.length
				) {
					currentDeck.data.slides.splice(
						detail.to,
						0,
						...currentDeck.data.slides.splice(detail.from, 1)
					);
	...
	private async emitSlideDidUpdate(element: HTMLElement) {
		const slideDidUpdate: CustomEvent<HTMLElement> =
			new CustomEvent<HTMLElement>('slideDidUpdate', {
				bubbles: true,
				detail: element,
			});
		...
		elements.forEach((e: HTMLElement) => {
			if (e.nodeName && e.nodeType === 1 && e.hasAttribute('slot')) {
				if (SlotUtils.isNodeEditable(e)) {
					...
					const editableElements = e.querySelectorAll('section');
					editableElements.forEach((element: HTMLElement) => {
						element.setAttribute('contentEditable', attrEditableValue);
						element.setAttribute('spellcheck', attrEditableValue);
					});
				}
			}
		});
