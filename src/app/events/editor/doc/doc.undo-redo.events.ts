...
export class DocUndoRedoEvents {
  ...
  private onFocusIn = ({ target }: FocusEvent) => {
    const focusedElement: HTMLElement | undefined | null =
      NodeUtils.toHTMLElement(target as Node);

    if (
      !focusedElement ||
      focusedElement.nodeName.toLowerCase() !== SlotType.CODE
    ) {
      return;
    }
  ...
  private async undoRedo({ undoRedo }: { undoRedo: () => Promise<void> }) {
    ...
    changeObserver.observe(this.containerRef, {
      characterData: true,
      subtree: true,
    });
  ...
  private observe() {
    ...
    this.inputObserver.observe(this.containerRef, {
      characterData: true,
      subtree: true,
      characterDataOldValue: true,
    });
    this.updateObserver.observe(this.containerRef, {
      childList: true,
      subtree: true,
    });
  ...
  private toUpdateParagraphs(
    paragraphs: HTMLElement[]
  ): UndoUpdateParagraphs[] {
  ...
  private onCharacterDataMutation = (mutations: MutationRecord[]) => {
    if (!this.undoInput) {
      ...
      const paragraph: HTMLElement | undefined = NodeUtils.toHTMLElement(
        findParagraph({ element: target, container: this.containerRef })
      );
      ...
      this.undoInput = {
        ...
        offset:
          ...
          (mutation.oldValue.length - newValue.length),
        index: elementIndex(paragraph),
  ...
  private onTreeMutation = (mutations: MutationRecord[]) => {
    ...
    addedParagraphs.forEach((paragraph: HTMLElement) =>
      changes.push({
        ...
        mutation: 'add',
        index: paragraph.previousSibling
          ? elementIndex(NodeUtils.toHTMLElement(paragraph.previousSibling)) + 1
          : 0,
      })
    );

    const addedIndex: number[] = changes.map(
      ({ index }: UndoRedoDocAddRemoveParagraph) => index
    );
    ...
    const lowerIndex: number = Math.min(
      ...removedParagraphs.map(({ previousSibling }: RemovedParagraph) =>
        previousSibling
          ? elementIndex(NodeUtils.toHTMLElement(previousSibling)) + 1
          : 0
      )
    );
    ...
    removedParagraphs
      .filter(
        (_removedParagraph: RemovedParagraph, index: number) =>
          !addedIndex.includes(index + lowerIndex)
      )
  ...
  private onUpdateMutation = (mutations: MutationRecord[]) => {
    ...
    const needsUpdate: boolean =
      addedNodesMutations.length > 0 || removedNodesMutations.length > 0;
    ...
    stackUndoUpdate({
      paragraphs: this.undoUpdateParagraphs.filter(
        ({ paragraph }: UndoUpdateParagraphs) => paragraph.isConnected
      ),
      container: this.containerRef,
    });
