...
import {
	createOfflineDoc,
	updateOfflineDoc,
} from '../../../providers/data/doc/doc.offline.provider';
import {
...
export class DocDataEvents {
	...
	private readonly debounceUpdateInput: () => void = debounce(
		async () => await this.updateData(),
		500
	);

	private debounceBusyEnd: () => void = debounce(
	...
	init(containerRef: HTMLElement) {
		...
		this.treeObserver.observe(this.containerRef, {
			childList: true,
			subtree: true,
		});

		this.attributesObserver = new MutationObserver(this.onAttributesMutation);
		this.attributesObserver.observe(this.containerRef, {
			attributes: true,
			subtree: true,
		});

		this.dataObserver = new MutationObserver(this.onDataMutation);
		this.dataObserver.observe(this.containerRef, {
			characterData: true,
			subtree: true,
		});
	...
	private onAttributesMutation = async (mutations: MutationRecord[]) => {
		...
		await this.updateParagraphs(
			mutations.filter(({ attributeName }: MutationRecord) =>
	...
	private onCustomEventChange = async ({
		detail,
	...
	private async createParagraph(element: HTMLElement) {
		...
		await this.updateDocParagraphList({
			paragraphId,
	...
	private async deleteParagraph(element: Node): Promise<string | undefined> {
		...
		const paragraphId: string = (element as HTMLElement).getAttribute(
			'paragraph_id'
		);
	...
	private postParagraph(element: HTMLElement): Promise<Paragraph> {
		return new Promise<Paragraph>(async (resolve) => {
			...
			> | null = this.paragraphAttributes(element);
			if (attributes) {
	...
	private updateDocParagraphList({
		paragraphId,
		paragraphElement,
	}: {
		paragraphId: string;
		...
		return new Promise<void>(async (resolve, reject) => {
			try {
				...
				doc.data.paragraphs = [
					...doc.data.paragraphs.slice(0, index),
					paragraphId,
					...doc.data.paragraphs.slice(index),
				];
	...
	private filterDocParagraphList(
		...
		return new Promise<void>(async (resolve, reject) => {
			try {
				...
				const filterParagraphIds: string[] = paragraphIds.filter(
				...
				doc.data.paragraphs = [
					...doc.data.paragraphs.filter(
						(paragraphId: string) => !filterParagraphIds.includes(paragraphId)
					),
				];
	...
	private async deleteParagraphs(mutations: MutationRecord[]) {
		try {
			...
			const removedNodes: Node[] = mutations.reduce(
				(acc: Node[], { removedNodes }: MutationRecord) => [
					...acc,
			...
			const promises: Promise<string | undefined>[] = removedNodes.map(
				(node: Node) => this.deleteParagraph(node)
			);
			const removedParagraphIds: (string | undefined)[] = await Promise.all(
				promises
			);
	...
	private async updateAddedNodesParagraphs(mutations: MutationRecord[]) {
		try {
			...
			const removedNodesMutations: MutationRecord[] =
				findRemovedNodesParagraphs({ mutations, container: this.containerRef });

			await this.updateParagraphs([
				...addedNodesMutations,
				...removedNodesMutations,
			]);
		} catch (err) {
			errorStore.state.error = err;
		}
	...
	private async updateParagraphs(mutations: MutationRecord[]) {
		try {
			...
			const promises: Promise<void>[] = updateParagraphs.map(
				(paragraph: HTMLElement) => this.updateParagraph(paragraph)
			);
	...
	private paragraphContent(paragraph: HTMLElement): string[] {
		return Array.from(paragraph.childNodes).reduce(
