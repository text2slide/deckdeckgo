...
import { Deck, AuthUser, formatDate } from '@deckdeckgo/editor';
...
} from '../../../utils/core/dashboard.utils';
...
import { GetAllRtData } from '../../../types/editor/deckAndSlide-model';
import { AppNavigationStart } from '../../../components/core/nav/app-navigation-start/app-navigation-start';
...
export class AppDecks implements ComponentInterface {
	...
	private readonly debounceLoading: () => void = debounce(
		() => (this.loading = false),
		750
	);
	private readonly debounceDecksLoading: () => void = debounce(
	...
	private static instance: AppDecks;

	static getInstance(): AppDecks {
		if (!AppDecks.instance) {
			AppDecks.instance = new AppDecks();
		}
		return AppDecks.instance;
	}
	...
	private async initDashboard() {
		...
		try {
			// /* *NOTE* API /getAllDecks */
			// const getAllDeckRt = await fetchData(ApiUrls.getAllDeck);

			// const deckList = getAllDeckRt.decksModel.map(
			// 	(deck) =>
			// 		({
			// 			id: deck.id,
			// 			data: deck.data,
	...
	private async fetchFirstSlides(decks: Deck[]): Promise<DeckAndFirstSlide[]> {
		...
		const promises: Promise<DeckAndFirstSlide>[] = decks
			.filter(
				(deck: Deck) => deck.data && deck.data.slides && deck.data.slides.length
			)
			.map((deck: Deck) =>
				this.initDeckAndFirstSlide(deck, deck.data.slides[0])
			);

		return Promise.all(promises);
	}

	private async initDeckAndFirstSlide(
		deck: Deck,
		slideId: string
	): Promise<DeckAndFirstSlide> {
		try {
			const slide: Slide = await getSlide(deck.id, slideId);

			const element: JSX.IntrinsicElements = await ParseSlidesUtils.parseSlide(
				slide,
				false
			);

			const style: Record<string, string | undefined> | undefined =
				this.convertStyle(deck);

			const background: JSX.IntrinsicElements | undefined =
				await ParseDeckSlotsUtils.convert(deck.data.background, 'background');
			const header: JSX.IntrinsicElements | undefined =
				await ParseDeckSlotsUtils.convert(deck.data.header, 'header');
			const footer: JSX.IntrinsicElements | undefined =
				await ParseDeckSlotsUtils.convert(deck.data.footer, 'footer');
	...
	private filterDecks(value: string | null) {
		...
		const matchingDecks: DeckAndFirstSlide[] = this.decks.filter(
			(matchDeck: DeckAndFirstSlide) => {
				return (
					matchDeck.deck?.data?.name
						?.toLowerCase()
	...
	private handleDate(deck: DeckAndFirstSlide): Date | number | BigInt {
		if (deck.deck.data.updated_at === null) {
			return deck.deck.data.created_at;
		} else {
			return deck.deck.data.updated_at;
		}
	}
	...
	private renderContent() {
		...
		if (!authStore.state.authUser) {
			return (
				<AppAnonymousContent
					title={i18n.state.menu.presentations}
					text={i18n.state.settings.access_decks}></AppAnonymousContent>
			);
		}

		return (
			<main class='ion-padding fit'>{this.renderYourPresentations()}</main>
		);
	}
			...
			return (
				<ion-searchbar
					...
					onIonChange={($event: CustomEvent) =>
						this.filterDecksOnChange($event)
					}
	...
	private renderDecks() {
		if (this.filteredDecks?.length > 0) {
			return (
				<div class={`container ${this.decksLoading ? 'loading' : ''}`}>
		...
		return this.filteredDecks.map((deck: DeckAndFirstSlide) => {
			...
			return (
				<article key={deck.deck.id}>
					<ion-card
						custom-tappable
						class='item ion-no-margin'
						onClick={() => this.navigateDeck(deck)}>
	...
	private renderAside(deck: DeckAndFirstSlide) {
		return (
			<aside>
				...
				<p>{formatDate(this.handleDate(deck))}</p>
				...
				<app-dashboard-actions
					...
					onCloned={() => this.handleCloneEmit()}></app-dashboard-actions>
			</aside>
		);
	}
