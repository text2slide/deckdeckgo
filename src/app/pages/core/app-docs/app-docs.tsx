...
export class AppDocs implements ComponentInterface {
	...
	private async initDocAndParagraphs(doc: Doc): Promise<DocAndParagraphs> {
		...
		const parsedParagraphs: JSX.IntrinsicElements[] = await Promise.all(
			paragraphs.map((paragraph: Paragraph) =>
				ParseParagraphsUtils.parseParagraph({ paragraph })
			)
		);
	...
	private filterDocs(value: string | null) {
		...
		const matchingDocs: DocAndParagraphs[] = this.docs.filter(
			(matchDoc: DocAndParagraphs) => {
				return (
					matchDoc.doc?.data?.name?.toLowerCase().indexOf(value.toLowerCase()) >
					-1
				);
			}
		);
	...
	private renderContent() {
		...
		if (!authStore.state.authUser) {
			return (
				<AppAnonymousContent
					title={i18n.state.menu.documents}
					text={i18n.state.settings.access_docs}></AppAnonymousContent>
			);
		}
