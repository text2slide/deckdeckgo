...
import {
	...
	initTemplates,
	updateTemplate,
} from '../../../../providers/data/template/template.provider';
import {
	getUserId,
	getUserEmail,
	getUserLoginType,
			...
			if (template.id) {
				...
				templatesStore.state.user = [
					...templatesStore.state.user.map((mapTemplate: Template) =>
						mapTemplate.id === updatedTemplate.id
							? updatedTemplate
							: mapTemplate
					),
				];
			} else {
				const createdTemplate: Template = await createUserTemplate(
					template.data
				);
				templatesStore.state.user = [
					createdTemplate,
			...
			<Fragment>
				<ion-label>
					{renderI18n(i18n.state.settings.contribute_community, {
						...
						value: (
							<a
								href='https://deckdeckgo.com/en/contact/'
