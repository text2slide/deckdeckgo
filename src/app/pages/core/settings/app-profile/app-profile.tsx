...
export class AppProfile {
	...
	private validateNameInput() {
		...
		this.validName =
			this.user && this.user.data && UserUtils.validName(this.user.data.name);
		this.isValid();
	}
	...
	private validateEmailInput() {
		...
		this.validEmail =
			this.user && this.user.data && UserUtils.validEmail(this.user.data.email);
		this.isValid();
	}
	...
	private uploadProfilePicture(): Promise<void> {
		return new Promise<void>(async (resolve, reject) => {
			...
			// }
			// try {
			// 	const storageFile: UserFileModel = await uploadOnlineFile(
			// 		this.profilePicture,
			// 		'IMAGE',
			// 		524288
			// 	);
			// 	if (storageFile) {
			// 		this.user.data.photo_url = storageFile.path;
			// 	}
			// 	resolve();
			// } catch (err) {
	...
	private uploadCustomLogo(): Promise<void> {
		return new Promise<void>(async (resolve, reject) => {
			...
			// 	);
			// 	if (storageFile) {
			// 		this.user.data.social.custom_logo_url = storageFile.path;
			// 	}
			// 	resolve();
			// } catch (err) {
	...
	private async selectProfilePicture() {
		const filePicker: HTMLInputElement = this.el.querySelector(
		...
		return [
			...
			<ion-item>
				<ion-input
					...
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
		...
		return [
			...
			<div class='newsletter'>
				...
				<ion-checkbox
					...
					value='pepperoni'
					checked={
						this.user && this.user.data ? this.user.data.newsletter : false
					}
	...
	private renderUsername() {
		...
		return [
			...
			<ion-item>
				<ion-input
					value={
						this.user && this.user.data ? this.user.data.username : undefined
					}
					debounce={500}
					minlength={3}
					maxlength={32}
	...
	private renderSubmitForm() {
		return (
			<ion-button
				...
				disabled={
					!this.valid || this.saving || !this.user || !authStore.state.loggedIn
				}
				color='primary'
				shape='round'>
	...
	private renderDangerZone() {
		return [
			...
			<ion-button
				...
				disabled={
					this.saving || !authStore.state.authUser || !authStore.state.loggedIn
				}>
		...
		return (
			<ion-list class='inputs-list'>
				...
				<div class='avatar'>
					<app-avatar
						src={
							this.user && this.user.data ? this.user.data.photo_url : undefined
						}
						aria-label='Profile picture'></app-avatar>
					<input
	...
	private renderTwitter() {
		return [
			...
			<p>
				<small>
					https://twitter.com/
					<strong>{this.twitter ? this.twitter : 'yourusername'}</strong>
				</small>
			</p>,
			<ion-item>
				<ion-input
					value={
						this.user && this.user.data && this.user.data.social
							? this.user.data.social.twitter
							: undefined
					}
					debounce={500}
					maxlength={128}
					...
					disabled={this.saving || !authStore.state.loggedIn}
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.handleSocialInput($event, 'twitter')
					}></ion-input>
			</ion-item>,
		];
	}
		...
		return [
			...
			<p>
				<small>
					...
					<strong>{this.linkedin ? this.linkedin : 'yourusername'}</strong>
				</small>
			</p>,
			<ion-item>
				<ion-input
					value={
						this.user && this.user.data && this.user.data.social
							? this.user.data.social.linkedin
							: undefined
					}
					debounce={500}
					maxlength={128}
					...
					disabled={this.saving || !authStore.state.loggedIn}
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.handleSocialInput($event, 'linkedin')
					}></ion-input>
			</ion-item>,
		];
	}
		...
		return [
			...
			<ion-item>
				<ion-input
					...
					disabled={this.saving || !authStore.state.loggedIn}
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.handleSocialInput($event, 'dev')
					}></ion-input>
			</ion-item>,
		];
	}
	...
	private renderMedium() {
		return [
			...
			<p>
				<small>
					https://medium.com/@
					<strong>{this.medium ? this.medium : 'yourusername'}</strong>
				</small>
			</p>,
			<ion-item>
				<ion-input
					...
					disabled={this.saving || !authStore.state.loggedIn}
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.handleSocialInput($event, 'medium')
					}></ion-input>
			</ion-item>,
		];
	}
	...
	private renderGithub() {
		return [
			...
			<p>
				<small>
					https://github.com/
					<strong>{this.github ? this.github : 'yourusername'}</strong>
				</small>
			</p>,
			<ion-item>
				<ion-input
					...
					disabled={this.saving || !authStore.state.loggedIn}
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.handleSocialInput($event, 'github')
					}></ion-input>
			</ion-item>,
		];
	}
		...
		return [
			...
			<p>
				<small>
					{renderI18n(i18n.state.settings.custom_url, {
						...
						value: (
							<strong>
								{this.custom ? this.custom : 'https://yourwebsite.com'}
							</strong>
						),
					})}
				</small>
			...
			<ion-item>
				<ion-input
					...
					disabled={this.saving || !authStore.state.loggedIn}
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.handleSocialInput($event, 'custom')
					}></ion-input>
			</ion-item>,
	...
	private renderCustomLogo() {
		return [
			...
			<div class='avatar'>
				...
				this.user.data &&
				this.user.data.social &&
				this.user.data.social.custom_logo_url ? (
					<deckgo-lazy-img
						slot='icon'
