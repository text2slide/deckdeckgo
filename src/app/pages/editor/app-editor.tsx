...
import { getEdit, setEditDeckId } from '../../utils/editor/editor.utils';
...
export class AppDeckEditor implements ComponentInterface {
	...
	render() {
		return (
			<Fragment>
				<app-navigation
					actions='all'
					class={this.hideNavigation ? 'hidden' : undefined}></app-navigation>
	...
	private renderEditor() {
		...
		return this.type === 'doc' ? (
			<app-doc-editor
				ref={(el) =>
					(this.docEditorRef = el as HTMLAppDocEditorElement)
				}></app-doc-editor>
		) : (
			<app-deck-editor
				ref={(el) =>
					(this.deckEditorRef = el as HTMLAppDeckEditorElement)
				}></app-deck-editor>
		);
	}
}
