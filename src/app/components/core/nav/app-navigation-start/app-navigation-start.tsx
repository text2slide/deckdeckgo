import { Component, h, Fragment, Element, Method } from '@stencil/core';
...
import { printDeck, printDoc } from '../../../../utils/editor/print.utils';
import { exportPPT } from '../../../../utils/editor/pptx.utils';
...
export class AppNavigationStart {
	...
	async print() {
		try {
			// printDoc(<deckgo-deck />);
			printDeck();
			// await FileSystemService.getInstance().print();
		} catch (err) {
			errorStore.state.error = `Something went wrong. ${err}.`;
		}
	} // print
	...
	private async importData() {
		...
		try {
			const type: 'doc' | 'deck' =
				await FileSystemService.getInstance().importData(
	...
	private async newType(type: 'deck' | 'doc') {
		...
		try {
			...
			errorStore.state.error =
				'Something went wrong while cleaning the local data.';
		}
	...
	private emitReloadEditor(detail: 'deck' | 'doc') {
		const initNewEditor: CustomEvent<'deck' | 'doc'> = new CustomEvent<
			'deck' | 'doc'
		>('reloadEditor', {
			bubbles: true,
			detail,
	...
	private renderActions() {
		return (
			<Fragment>
				...
				<button
					key='open-file-action'
					class='ion-activatable'
					onClick={async ($event: UIEvent) =>
						await this.openFilePicker($event)
					}>
					<ion-ripple-effect></ion-ripple-effect>
				...
				{/* <button
					key='export-action'
					class='ion-activatable'
					onClick={async () => await this.exportData()}>
					<ion-ripple-effect></ion-ripple-effect>
				...
				<button
					key='export-ppt'
					class='ion-activatable'
					onClick={async () => await this.exportPPT()}>
					<ion-ripple-effect></ion-ripple-effect>
					<AppIcon name='ppt' ariaHidden={true} ariaLabel=''></AppIcon>
					<ion-label>{i18n.state.editor.export}</ion-label>
				</button>
				...
				<button
					...
					onClick={async ($event: UIEvent) =>
						await this.openMoreActions($event)
					}
					color='primary'
