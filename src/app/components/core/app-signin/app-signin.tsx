...
import {
	EnvironmentCloud,
	EnvironmentDeckDeckGoConfig,
} from '../../../types/core/environment-config';
...
export class AppSignIn {
	...
	private deckDeckGoConfig: EnvironmentDeckDeckGoConfig | undefined =
	...
	private onSignInSuccess = (
		credentials:
			| { uid: string | undefined; githubAccessToken: string | undefined }
			| undefined
	) => {
	...
	private onSignInError = (err?: string) => {
		...
		errorStore.state.error =
			'There was an issue sign in with the internet identity.';
	};

	private navigateRedirect() {
		navStore.state.nav = {
			url: this.deckDeckGoConfig.navigateReloadEditorUrl,
			direction: NavDirection.RELOAD,
		};
	}

	private saveGithubCredentials(
		credentials:
			| { uid: string | undefined; githubAccessToken: string | undefined }
			| undefined
	) {
	...
	private async loadSignIn() {
		...
		const { signIn }: EnvironmentCloud =
			EnvironmentConfigService.getInstance().get('cloud');
	...
	// /* *NOTE* authStore.state.authUser data need to be send when user logged in */
	// async testLogin() {
	// 	// add authStore.state.authUser
	...
	// 		uid: '73d56482-da74-4b1b-89e0-8e726de54664',
	// 		githubAccessToken: '',
	// 	});
	// }
		...
		return (
			<ion-buttons class='back'>
				<ion-button
					...
					color='dark'
					aria-label={i18n.state.core.close}
					disabled={this.signInInProgress}>
	...
	private renderMsg() {
		return [
			<h1 class='ion-text-center ion-padding-start ion-padding-end'>
