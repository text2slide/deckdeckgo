...
export class AppAssetImage implements ComponentInterface {
	...
	render() {
		...
		return (
			<Host class={`${this.imgLoaded || wave ? 'imgLoaded' : ''}`}>
				{this.renderImage()}
			</Host>
		);
	}
	...
	private renderPhotoCredits(photo: UnsplashPhoto) {
		...
		return (
			<ion-label class='photo-credits'>
				<a
					href={
						photo.user.links.html + '?utm_source=DeckDeckGo&utm_medium=referral'
					}
					target='_blank'
		...
		if (storageFile && storageFile.downloadUrl) {
			return (
				<Fragment>
					<div class='image-container'>
						<deckgo-lazy-img
							imgSrc={storageFile.downloadUrl}
							imgAlt={storageFile.name}
							custom-loader={true}></deckgo-lazy-img>
					</div>
				</Fragment>
			);
