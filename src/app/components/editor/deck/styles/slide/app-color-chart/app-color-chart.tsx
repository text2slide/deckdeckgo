...
export class AppColorDeckSlide {
  ...
  private initColor = async (): Promise<InitStyleColor> => {
    ...
    if (this.applyColorType === ApplyColorType.FILL) {
      return ColorUtils.splitColor(
        this.hexOrRgb(
          this.selectedElement.style.getPropertyValue(
            `--deckgo-chart-fill-color-${this.colorIndex}`
          )
        )
      );
      ...
      return ColorUtils.splitColor(
        this.hexOrRgb(
          this.selectedElement.style.getPropertyValue(
            `--deckgo-chart-stroke-${this.colorIndex}`
          )
        )
      );
    } else if (this.applyColorType === ApplyColorType.AXIS) {
      return ColorUtils.splitColor(
        this.selectedElement.style.getPropertyValue('--deckgo-chart-axis-color')
      );
    } else if (this.applyColorType === ApplyColorType.GRID) {
      return ColorUtils.splitColor(
        this.selectedElement.style.getPropertyValue(
          '--deckgo-chart-grid-stroke'
        )
      );
    } else {
      return ColorUtils.splitColor(
        this.selectedElement.style.getPropertyValue('--deckgo-chart-text-color')
      );
    }
  };
  ...
  private async resetColor() {
    ...
    if (this.applyColorType === ApplyColorType.FILL) {
      this.selectedElement.style.removeProperty(
        `--deckgo-chart-fill-color-${this.colorIndex}`
      );
    } else if (this.applyColorType === ApplyColorType.STROKE) {
      this.selectedElement.style.removeProperty(
        ...
        <ion-list>
          ...
          <ion-item class='select'>
            ...
            <ion-select
              ...
              disabled={
                this.applyColorType !== ApplyColorType.FILL &&
                this.applyColorType !== ApplyColorType.STROKE
              }
  ...
  private renderColorOptions() {
    const options = [
      <ion-select-option value={ApplyColorType.FILL}>
        {i18n.state.editor.fill}
      </ion-select-option>,
      <ion-select-option value={ApplyColorType.STROKE}>
        {i18n.state.editor.stroke}
      </ion-select-option>,
      <ion-select-option value={ApplyColorType.FONT}>
    ...
    if (this.chartType != SlideChartType.PIE) {
      options.push(
        <ion-select-option value={ApplyColorType.AXIS}>
          {i18n.state.editor.axis}
        </ion-select-option>
      );
      options.push(
        <ion-select-option value={ApplyColorType.GRID}>
  ...
  private renderChartIndexes() {
    return this.indexes.map((index: number) => {
      return (
        <ion-select-option value={index + 1}>{`${
          index + 1
        }`}</ion-select-option>
      );
    });
