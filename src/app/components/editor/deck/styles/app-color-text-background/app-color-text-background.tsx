...
export class AppColorTextBackground {
	...
	private async applyTextColor(selectedColor: string) {
		...
		setStyle(this.selectedElement, {
			properties: [
				{
					property: this.deck || this.slide ? '--color' : 'color',
					value: selectedColor,
				},
			],
	...
	private async applyBackground(selectedColor: string) {
		...
		setStyle(this.selectedElement, {
			properties: [
				{
					value: selectedColor,
					property: this.deck || this.slide ? '--background' : 'background',
				},
			],
			type: this.deck ? 'deck' : this.slide ? 'slide' : 'element',
			updateUI: async (_value: string) => await this.colorRef.loadColor(),
	...
	render() {
		return (
			<app-expansion-panel
				expanded={
					this.colorType === 'text'
						? settingsStore.state.panels.color
						: settingsStore.state.panels.background
				}
