...
export class AppBlock {
	...
	private async initRotate() {
		const matches: RegExpMatchArray | null =
			this.selectedElement?.element?.style.transform.match(/(\d+)/);
	...
	private updateStyle({
		...
		property: 'transform' | 'width' | 'padding';
		value: string;
	}) {
	...
	private renderPadding() {
		return (
			<Fragment>
				<ion-item-divider class='ion-padding-top'>
					<ion-label>
						{i18n.state.editor.padding}{' '}
						{settingsStore.state.editMode === 'properties' ? (
							<small>{this.padding}px</small>
						) : undefined}
					</ion-label>
				</ion-item-divider>
