...
export class AppColor {
	...
	private readonly debounceHandleHexInput: (
	...
	private readonly debounceHandleRgbInput: (
		$event: CustomEvent<KeyboardEvent>,
		colorType: 'r' | 'g' | 'b'
	) => void = debounce(
		($event: CustomEvent<KeyboardEvent>, colorType: 'r' | 'g' | 'b') =>
			this.handleRgbInput($event, colorType),
		500
	);
	...
	private colorChange() {
		this.colorDidChange.emit(
			`rgba(${this.color.rgb.value},${ColorUtils.transformOpacity(
				this.opacity
			)})`
		);
		...
		return (
			<div class='color-picker item-input'>
				...
				<button
					slot='end'
					class='reset'
					arial-label={i18n.state.core.reset}
					onClick={($event: UIEvent) => this.emitReset($event)}>
			...
			return (
				<ion-input
					...
					onIonInput={($event: CustomEvent<KeyboardEvent>) =>
						this.debounceHandleHexInput($event)
					}
					required={true}
			...
			return (
				<div class='input-rgb'>
					<ion-input
						...
						max-length={3}
						onIonInput={($event: CustomEvent<KeyboardEvent>) =>
							this.debounceHandleRgbInput($event, 'r')
						}
						min={'0'}
						max={'255'}
						name='r'
					...
					<ion-input
						...
						max-length={3}
						onIonInput={($event: CustomEvent<KeyboardEvent>) =>
							this.debounceHandleRgbInput($event, 'g')
						}
						min={'0'}
						max={'255'}
						name='g'
					...
					<ion-input
						...
						max-length={3}
						onIonInput={($event: CustomEvent<KeyboardEvent>) =>
							this.debounceHandleRgbInput($event, 'b')
						}
						min={'0'}
						max={'255'}
						name='b'
	...
	private renderColorSwitcher() {
		return (
			<div class={`switcher ${colorStore.state.colorInput}`}>
				<button
					onClick={($event: UIEvent) => this.switchInputColor($event, 'hex')}>
					hex
				</button>{' '}
				<button
					onClick={($event: UIEvent) => this.switchInputColor($event, 'rgb')}>
					rgb
				</button>
			</div>
		);
			...
			<Fragment>
				...
				<ion-item class='item-range'>
					<ion-range
						...
						max={100}
						disabled={
							!this.color || this.color === undefined || !this.color.hex
						}
						value={this.opacity}
						mode='md'
	...
	private renderColorHistory() {
		return (
			<div class='history ion-padding-start ion-padding-end ion-padding-bottom'>
				{this.renderPalette()}
			</div>
		);
	}
		...
		return colorStore.state.history.map((palette: DeckdeckgoPalette) => {
			return (
				<ion-fab-button
					size='small'
					style={{
						'--background': palette.color.hex,
						'--background-hover': palette.color.hex,
						'--background-activated': palette.color.hex,
					}}
					onClick={($event: UIEvent) =>
						this.selectColor($event, palette.color)
					}></ion-fab-button>
			);
		});
	}
