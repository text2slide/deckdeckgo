...
export class AppImageHistory {
	...
	private async initImagesHistory() {
		let imagesHistory: (UnsplashPhoto | TenorGif | StorageFile | Waves)[] =
			await this.imageHistoryService.get();
		...
		if (!this.deck && !this.slide) {
			imagesHistory = [
				...imagesHistory.filter((img) => !img.hasOwnProperty('viewBox')),
			];
		}
	...
	private renderImages() {
		return (
			<div class='history-photos ion-padding'>
				{this.imagesHistory.map(
					(
						history: UnsplashPhoto | TenorGif | StorageFile | Waves,
						index: number
					) => (
						<article
							custom-tappable
							key={`history-${index}`}
							onClick={() => this.selectImage.emit(history)}>
