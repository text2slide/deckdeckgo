...
export class AppImageChoice {
	...
	private async selectImageFromHistory(
		$event: CustomEvent<UnsplashPhoto | TenorGif | StorageFile | Waves>
	): Promise<void> {
		...
		return (
			<app-expansion-panel
				...
				<div class='image-actions ion-margin'>
					<AppAssetChoice
						selectAction={async (
							action: EditAction,
							image?: UnsplashPhoto | TenorGif | StorageFile | Waves
						) => await this.selectAction(action, image)}></AppAssetChoice>
					{this.renderWaves()}
				...
				<app-image-history
					...
					onSelectImage={async (
						$event: CustomEvent<UnsplashPhoto | TenorGif | StorageFile | Waves>
					) => await this.selectImageFromHistory($event)}></app-image-history>
			</app-expansion-panel>
		);
	}
	...
	private renderWaves() {
		...
		return (
			<ion-button
				shape='round'
				onClick={async () => await this.selectAction(EditAction.OPEN_SVG_WAVES)}
				color='quaternary'>
		...
		if ((!this.deck && !this.slide) || !this.deleteBackground) {
			...
			return (
				<ion-button
					shape='round'
					onClick={async () =>
						await this.selectAction(EditAction.DELETE_BACKGROUND)
					}
					fill='outline'
					class='delete'>
