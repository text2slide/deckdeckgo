...
export class AppDeckHeaderFooter {
	...
	async componentWillLoad() {
		this.headerType = await HeaderFooterUtils.currentType(
			this.deckElement,
			'header'
		);
		this.footerType = await HeaderFooterUtils.currentType(
			this.deckElement,
			'footer'
		);
	}

	private async appendHeaderFooter(
		...
		await HeaderFooterUtils.append(
			this.deckElement,
	...
	private hasOneOptionAtLeast(): boolean {
		return (
			...
			(userStore.state.user.data.social.twitter !== null ||
				...
				userStore.state.user.data.social.pageNumber !== null)
		);
	}
				...
				<div class='container ion-margin-bottom'>
					<ion-label class='no-options'>
						{renderI18n(i18n.state.editor.header_footer_sign_in, {
							placeholder: '{0}',
							value: (
								<a onClick={() => this.onNavigateSignIn()}>
									{i18n.state.nav.sign_in.toLowerCase()}
								</a>
							),
						})}
					</ion-label>
		...
		return (
			<app-expansion-panel>
				...
				<div class='container ion-margin-bottom'>
					...
					{this.renderPageNumber(slotName, selectedType)}
	...
	/* *NOTE* footer add render page number */
	private renderPageNumber(
		...
		if (
			!userStore.state.user.data.social.pageNumber ||
			userStore.state.user.data.social.pageNumber === '' ||
			userStore.state.user.data.social.pageNumber === undefined
		) {
			return undefined;
		}
		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'pageNumber')}
				class={selectedType === 'pageNumber' ? 'selected' : undefined}>
				Page Number
			</button>
		);
	}
	...
	private renderNoOptions(options: boolean) {
		...
		return (
			<ion-label class='no-options'>{i18n.state.editor.no_options}</ion-label>
		);
	}
	...
	private renderTwitter(
		...
		const link: JSX.IntrinsicElements = SocialUtils.createTwitter(
			userStore.state.user
		);

		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'twitter')}
				class={selectedType === 'twitter' ? 'selected' : undefined}>
				{link}
			</button>
		);
	}
	...
	private renderLinkedin(
		...
		const link: JSX.IntrinsicElements = SocialUtils.createLinkedin(
			userStore.state.user
		);

		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'linkedin')}
				class={selectedType === 'linkedin' ? 'selected' : undefined}>
				{link}
			</button>
		);
	}
	...
	private renderDev(
		...
		const link: JSX.IntrinsicElements = SocialUtils.createDev(
			userStore.state.user
		);

		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'dev')}
				class={selectedType === 'dev' ? 'selected' : undefined}>
				{link}
			</button>
		);
	}
	...
	private renderMedium(
		...
		const link: JSX.IntrinsicElements = SocialUtils.createMedium(
			userStore.state.user
		);

		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'medium')}
				class={selectedType === 'medium' ? 'selected' : undefined}>
				{link}
			</button>
		);
	}
	...
	private renderGitHub(
		...
		const link: JSX.IntrinsicElements = SocialUtils.createGitHub(
			userStore.state.user
		);

		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'github')}
				class={selectedType === 'github' ? 'selected' : undefined}>
				{link}
			</button>
		);
	}
	...
	private renderCustom(
		...
		const link: JSX.IntrinsicElements = SocialUtils.createCustom(
			userStore.state.user
		);

		return (
			<button
				onClick={() => this.appendHeaderFooter(slotName, 'custom')}
				class={selectedType === 'custom' ? 'selected' : undefined}>
				{link}
			</button>
		);
	}
		...
		return (
			<div class='ion-padding-start ion-padding-end ion-padding-top'>
				{renderI18n(i18n.state.editor.header_footer_edit, {
					placeholder: '{0}',
					value: (
						<a onClick={() => this.onNavigateSettings()}>
							{i18n.state.nav.settings.toLowerCase()}
						</a>
					),
				})}
			</div>
