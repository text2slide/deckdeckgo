...
export class AppActionsDeck {
	...
	async componentWillLoad() {
		this.destroyListener = remoteStore.onChange(
			'pendingRequests',
			async (requests: DeckdeckgoEventDeckRequest[] | undefined) => {
	...
	private async openRemote() {
		...
		if (
			connected &&
			remoteStore.state.pendingRequests &&
			remoteStore.state.pendingRequests.length > 0
		) {
	...
	private async closeRemote() {
		...
		const popover: HTMLIonPopoverElement =
	...
	render() {
		// save slide number to local storage
		localStorage.setItem('currentSlideIndex', this.slideNumber.toString());

		return (
			<aside>
				<ion-buttons slot='start'>
					<app-action-add-slide
						currentSlideIndex={this.slideNumber}
						slidesLength={this.slides?.length}></app-action-add-slide>
					...
					<button
						...
						class={`ion-activatable wider-devices ${
							!this.mobile ? 'hide-slide-navigate' : ''
						}`}>
						<ion-ripple-effect></ion-ripple-effect>
