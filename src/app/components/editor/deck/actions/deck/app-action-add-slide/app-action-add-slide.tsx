...
export class AppActionAddSlide {
		...
		popover.onDidDismiss().then(async (detail: OverlayEventDetail) => {
			if (detail?.data) {
				if (detail.data.template === SlideTemplate.GIF) {
					...
					detail.data.template === SlideTemplate.SPLIT &&
					detail.data.attributes?.type === SlideSplitType.DEMO
				) {
					await this.openDemo();
	...
	private addSlideGif(gif: TenorGif): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			const slide: JSX.IntrinsicElements =
				await CreateSlidesUtils.createSlideGif(url);
	...
	private addSlideYoutube(youtubeUrl: string): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			const slide: JSX.IntrinsicElements =
				await CreateSlidesUtils.createSlideYoutube(youtubeUrl);
	...
	private async addSlidePlayground(playground: PlaygroundAction) {
		...
		const slide: JSX.IntrinsicElements =
			await CreateSlidesUtils.createSlidePlayground(
				playground.src,
				playground.theme
			);
	...
	private addSlideDemo(demo: DemoAction): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			const slide: JSX.IntrinsicElements =
				await CreateSlidesUtils.createSlideDemo(demo.src, demo.mode);
	...
	private addSlidePoll(question: string, answers: string[]): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			const slide: JSX.IntrinsicElements =
				await CreateSlidesUtils.createSlidePoll(question, answers);
	...
	private addSlideChart(
		...
		return new Promise<void>(async (resolve) => {
			...
			const slide: JSX.IntrinsicElements =
				await CreateSlidesUtils.createSlideChart(attributes);
	...
	render() {
		return (
			<app-action-busy
				...
				iconName='add'
				onActionReady={($event: CustomEvent) =>
