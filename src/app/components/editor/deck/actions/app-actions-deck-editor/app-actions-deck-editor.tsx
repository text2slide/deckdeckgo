...
export class AppActionsDeckEditor {
	...
	componentDidLoad() {
		this.destroyUndoRedoListener = undoRedoStore.onChange(
			'elementInnerHTML',
			(elementInnerHTML: string | undefined) => {
				if (elementInnerHTML === undefined) {
					this.el.removeEventListener(
						'click',
						this.resetElementInnerHTML,
						false
					);
					return;
				}

				this.el.addEventListener('click', this.resetElementInnerHTML, {
					once: true,
				});
			}
		);
	...
	async touch(element: HTMLElement, autoOpen: boolean = true) {
		...
		deckEditorStore.state.step = isSlide(element)
			? BreadcrumbsStep.SLIDE
			: BreadcrumbsStep.ELEMENT;
	}
	...
	private renderFullscreen() {
		return (
			<app-bottom-sheet
				onSheetChanged={($event: CustomEvent<'open' | 'close'>) =>
					this.sheetChanged($event)
				}>
				<app-breadcrumbs
					slideNumber={this.slideNumber}
					onStepTo={($event: CustomEvent<HTMLElement>) =>
						this.selectStep($event?.detail)
					}></app-breadcrumbs>
		...
		return (
			<app-actions-element
				onClick={($event: MouseEvent | TouchEvent) =>
					this.selectDeckOnClick($event)
				}
				ref={(el) =>
					(this.actionsElementRef = el as HTMLAppActionsElementElement)
				}
				class={
					deckEditorStore.state.step === BreadcrumbsStep.DECK
						? 'hidden'
				...
				activeSlide={this.slides[this.slideNumber]}></app-actions-element>
		);
	}
}
