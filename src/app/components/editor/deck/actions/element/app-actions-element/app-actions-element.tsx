...
export class AppActionsElement {
	...
	private initWindowResize() {
		if (window) {
			window.addEventListener(
				'resize',
				() => this.debounceResizeSlideContent(),
				{
					passive: true,
				}
			);
	...
	@Listen('elementDidMoved', { target: 'document' })
	...
	async onToggleReveal($event: CustomEvent<boolean>) {
		...
		const element: HTMLElement = await RevealSlotUtils.toggleReveal(
	...
	async touch(element: HTMLElement | undefined, autoOpen: boolean = true) {
		...
		if (this.selectedElement?.type === 'element') {
			...
			this.observer.observe(this.selectedElement.element, {
				attributes: true,
				childList: true,
				subtree: true,
			});
	...
	private isImgNotDefined(element: HTMLElement): boolean {
		return (
			element?.nodeName?.toLowerCase() === SlotType.IMG &&
	...
	private findSelectedElement(element: HTMLElement): Promise<HTMLElement> {
		return new Promise<HTMLElement>(async (resolve) => {
			...
			const result: HTMLElement = await this.findSelectedElement(
	...
	private deleteElement(): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			if (this.selectedElement.type === 'slide') {
				...
				this.selectedElement.element.parentElement.removeChild(
					this.selectedElement.element
				);
	...
	private async openStackingOrder($event: UIEvent) {
		const popover: HTMLIonPopoverElement = await popoverController.create({
			component: 'app-stacking-order',
			componentProps: {
				selectedElement: this.selectedElement.element,
				activeSlideChildren: this.activeSlide,
			},
			mode: 'ios',
			event: $event,
		});
	...
	private async clone() {
		if (this.selectedElement?.slot?.shape || this.selectedElement?.slot?.text) {
	...
	private async openTransform() {
		if (
			...
			!this.selectedElement?.slide?.fixed
		) {
			return;
		}
	...
	private async openTitleShape(component: 'app-shape' | 'app-image-element') {
		if (this.selectedElement?.type === 'element') {
			return;
		}
		const popover: HTMLIonPopoverElement = await popoverController.create({
	...
	private async openShape(component: 'app-shape' | 'app-image-element') {
		if (
			this.selectedElement?.type === 'element' ||
			!this.selectedElement?.slide?.aspectRatio
		) {
			return;
		}
	...
	private async transformSlotType(type: SlotType) {
		...
		const element: HTMLElement = await ToggleSlotUtils.toggleSlotType(
			this.selectedElement.element,
			type
		);
	...
	private async transformTemplate(template: InitTemplate) {
		...
		const slide: JSX.IntrinsicElements = await CloneSlideUtils.toggleTemplate(
			this.selectedElement.element,
			template
		);
	...
	private async replaceSlot(element: HTMLElement) {
		...
		this.selectedElement.element.parentElement.replaceChild(
			element,
			this.selectedElement.element
		);
	...
	private async attachResizeSlideContent() {
		...
		if (window && 'ResizeObserver' in window) {
			...
			this.selectedElement.element.addEventListener(
				'focusout',
				() => this.debounceResizeSlideContent(),
				{ passive: true }
			);
	...
	private detachResizeSlideContent(): Promise<void> {
		return new Promise<void>((resolve) => {
			if (window && 'ResizeObserver' in window) {
				...
				this.selectedElement.element.removeEventListener(
					'focusout',
					() => this.debounceResizeSlideContent(),
					true
				);
	...
	private async openStyle() {
		const popover: HTMLIonPopoverElement = await popoverController.create({
			...
			showBackdrop: false,
			cssClass: `popover-menu ${
				this.selectedElement?.slide?.poll ? 'popover-menu-wide' : ''
			}`,
		});
	...
	private async openAlignment() {
		const popover: HTMLIonPopoverElement = await popoverController.create({
			component: 'app-element-alignment',
			componentProps: {
				selectedElement: this.selectedElement,
			},
			mode: 'ios',
			showBackdrop: false,
			cssClass: `popover-menu popover-alignment`,
		});
	...
	private async deleteLogo() {
		await this.imageHelper.deleteSlideAttributeImgSrc(
			this.selectedElement.element
		);
	}
	...
	private updateSlideDemo = async (demoAttr: DemoAction): Promise<void> => {
		...
		if (
			!demoAttr ||
			!demoAttr.src ||
			demoAttr.src === undefined ||
			demoAttr.src === ''
		) {
			return;
		}

		const demo: HTMLElement =
			this.selectedElement.element.querySelector('deckgo-demo');
	...
	private toggleList(
		destinationListType: SlotType.OL | SlotType.UL
	): Promise<void> {
		return new Promise<void>(async (resolve) => {
			if (!this.selectedElement) {
				resolve();
	...
	private renderNotes() {
		const classElement: string | undefined = `wider-devices ion-activatable ${
			this.selectedElement?.type === 'slide' ? '' : 'hidden'
		}`;
	...
	private renderCopyStyle() {
		const displayed: boolean =
			...
			!this.selectedElement?.slot?.shape &&
			!this.selectedElement?.slot?.text;
		const classSlide: string | undefined = `ion-activatable ${
			displayed ? '' : 'hidden'
		}`;
	...
	private renderStyle() {
		return [
			<button
				...
				aria-label={i18n.state.editor.style}
			...
			this.selectedElement?.type === 'element' ? (
				<button
					onClick={($event: UIEvent) => this.openStackingOrder($event)}
					aria-label={i18n.state.editor.stacking_order}
					disabled={
						store.state.busy && this.selectedElement?.type === 'element'
					}
					class='wider-devices ion-activatable'>
					<ion-ripple-effect></ion-ripple-effect>
					<AppIcon
					...
					<ion-label aria-hidden='true'>
						{i18n.state.editor.stacking_order}
					</ion-label>
				</button>
			) : null,
			this.selectedElement?.type === 'element' ? (
				<button
					...
					aria-label={i18n.state.editor.alignment}
					class='ion-activatable'>
					<ion-ripple-effect></ion-ripple-effect>
					<AppIcon name='alignment' ariaLabel='' ariaHidden={true}></AppIcon>
					<ion-label aria-hidden='true'>
	...
	private renderEdit() {
		const displayed: boolean =
			this.selectedElement?.type === 'slide' && this.isSlideEditable();
		const classSlide: string | undefined = `ion-activatable${
			displayed ? '' : ' hidden'
		}`;
	...
	private renderTransform() {
		...
		const classToggle: string | undefined = `wider-devices ion-activatable${
			displayed ? '' : ' hidden'
		}`;
	...
	private renderAspectRatio() {
		...
		return [
			<button
				...
				aria-label={i18n.state.editor.add_text}
				class={classSlide}
				tabindex={displayed ? 0 : -1}>
				<ion-ripple-effect></ion-ripple-effect>
				<AppIcon
	...
	private renderCodeOptions() {
		const classSlideCode: string | undefined = `ion-activatable${
			this.selectedElement?.slot?.code ? '' : ' hidden'
		}`;
	...
	private renderMathOptions() {
		const classSlideMath: string | undefined = `ion-activatable${
			this.selectedElement?.slot?.math ? '' : ' hidden'
		}`;
	...
	private renderImages() {
		const classImage: string | undefined = `ion-activatable${
			this.selectedElement?.slot?.image ? '' : ' hidden'
		}`;
