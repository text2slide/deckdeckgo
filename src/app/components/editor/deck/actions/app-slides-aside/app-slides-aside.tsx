...
export class AppSlidesAside {
	...
	@Event()
	private didReorder: EventEmitter<void>;
	...
	@Watch('activeIndex')
	onActiveIndexChange() {
		const nextThumbnail: HTMLElement | null = this.el.querySelector(
			`app-slide-thumbnail:nth-child(${this.activeIndex + 1})`
		);

		const rect: DOMRect | undefined = nextThumbnail?.getBoundingClientRect();

		if (
			rect?.top > this.hostRef?.scrollHeight ||
			rect?.top < this.hostRef?.scrollTop
		) {
	...
	async onSlidesDelete({ detail }: CustomEvent<HTMLElement[]>) {
		detail.forEach(async (deletedSlide) => {
			await this.deleteSlide(deletedSlide);
		});
		this.debounceUpdateAllSlides();
	}
	...
	private async deleteSlide(deletedSlide: HTMLElement) {
		const slideIndex: number = this.slides.findIndex(
			(slide) =>
				slide.getAttribute('slide_id') === deletedSlide.getAttribute('slide_id')
		);
		// elementIndex(deletedSlide);
		this.slides = [
			...this.slides.filter(
				(_slide: HTMLElement, index: number) => slideIndex !== index
			),
		];
	}
	...
	private async updateAllSlides() {
		...
		this.slides = Array.from(slides)
			.filter((slide: HTMLElement) => {
				return !slide.closest('.cloneForExport') && isSlide(slide);
			})
	...
	private renderThumbnail(slide: HTMLElement, index: number) {
		const dragClass: string =
			...
			this.reorderDetail?.from !== this.reorderDetail?.to
				? 'hover'
				: index === 0 && this.reorderDetail?.to === -1
				? this.reorderDetail?.from === index
					? 'drag-start'
					: 'hover-top'
		...
		return (
			<app-slide-thumbnail
				...
				onDragEnter={() => this.onDragHover(index)}></app-slide-thumbnail>
		);
	}

	private renderActions() {return (
			<div class='actions'>
				<app-action-add-slide
					currentSlideIndex={this.activeIndex}
					slidesLength={this.slides.length}
					popoverCssClass='popover-menu-wide-start'></app-action-add-slide>
			</div>
		);
	}
}
