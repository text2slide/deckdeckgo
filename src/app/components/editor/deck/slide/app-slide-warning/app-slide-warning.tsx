...
export class AppSlideWarning {
	...
	private readonly debounceResizeWarningOverflow: () => void = debounce(
		async () => {
	...
	private async hasOverflow(): Promise<boolean> {
		...
		const filteredSlots: HTMLElement[] = Array.from(slots).filter(
			(element: HTMLElement) => {
				return (
					!SlotUtils.isNodeEditable(element) ||
					SlotUtils.isNodeMarkdown(element)
				);
			}
		);
		...
		const promises: Promise<boolean>[] = Array.from(filteredSlots).map(
			(element: HTMLElement) => this.isOverflown(element, slide)
		);
		...
		const hasOverflow: boolean | undefined = overflows.find(
	...
	private async hasLowContrast(): Promise<boolean> {
		...
		const slotsWithText: HTMLElement[] = await NodeUtils.childrenTextNode(
			filteredSlots
		);
		...
		const promises: Promise<number>[] = Array.from(elements).map(
			(element: HTMLElement) => this.calculateRatio(element, deck, slide)
		);
		...
		const lowContrast: number | undefined = contrasts.find(
			(contrast: number) => contrast < this.lowestAACompliantLevel
		);
	...
	private async calculateRatio(
		...
		const bgColor = await NodeUtils.findColors(
			element,
			'background',
			deck,
			slide
	...
	private async isOverflown(
		element: HTMLElement,
		slide: HTMLElement
	): Promise<boolean> {
		if (typeof (element as any).getContainer === 'function') {
			const shadowedContainer: HTMLElement | null = await (
				element as any
			).getContainer();
	...
	private async isHTMLElementOverflown(
	...
	private async getCurrentSlide(): Promise<{
		deck: HTMLElement | null;
		slide: HTMLElement | null;
	}> {
		const deck: HTMLDeckgoDeckElement =
			document.querySelector('main > deckgo-deck');
	...
	private async openInformation($event: UIEvent) {
		const popover: HTMLIonPopoverElement = await popoverController.create({
			...
			componentProps: {
				lowContrast:
					settingsStore.state.contrastWarning && this.warningLowContrast,
		...
		return (
			<Host
				...
				<button
					...
					onClick={($event: UIEvent) => this.openInformation($event)}>
	...
	private renderMsg() {
		if (
			settingsStore.state.contrastWarning &&
			this.warningLowContrast &&
			this.warningOverflow
		) {
			return (
				<ion-label>
