...
export class AppSlideThumbnail {
  ...
  private async lazyLoadImages() {
    const images: NodeListOf<HTMLDeckgoLazyImgElement> =
      this.deckPreviewRef.querySelectorAll('deckgo-lazy-img');
    const promises: Promise<void>[] = Array.from(images).map(
  ...
  private async lazyLoadCharts() {
    const charts: NodeListOf<HTMLDeckgoSlideChartElement> =
      this.deckPreviewRef.querySelectorAll('deckgo-slide-chart-custom');
    const promises: Promise<void>[] = Array.from(charts).map(
      (chart: HTMLDeckgoSlideChartElement) => {
  ...
  private renderMiniature() {
    ...
    return (
      <deckgo-deck
        ...
        onDeckDidLoad={($event: CustomEvent) =>
          $event.stopPropagation()
        }></deckgo-deck>
    );
  }
