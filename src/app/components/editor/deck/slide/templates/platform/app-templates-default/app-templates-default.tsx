...
export class AppTemplatesDefault {
	...
	private async lazyLoadContent() {
		const slideGif: HTMLElement | null = this.el.querySelector(
			'deckgo-slide-gif.showcase'
		);
		const slideAuthor: HTMLElement | null = this.el.querySelector(
			'deckgo-slide-author.showcase'
		);
		const slideQRCode: HTMLElement | null = this.el.querySelector(
			'deckgo-slide-qrcode.showcase'
		);
		const slidesChart: HTMLElement | null = this.el.querySelector(
			'deckgo-slide-chart.showcase'
		);
		const slidesPoll: HTMLElement | null = this.el.querySelector(
			'deckgo-slide-poll.showcase'
		);
		...
		const slides: HTMLElement[] = [
			...
			slidesChart,
			slidesPoll,
		].filter((element: HTMLElement | null) => element !== null);
	...
	private async drawChart() {
		const slideChart: HTMLDeckgoSlideChartElement = this.el.querySelector(
			'deckgo-slide-chart.showcase'
		);
	...
	private async updatePollChart() {
		const slidePoll: HTMLDeckgoSlidePollElement = this.el.querySelector(
			'deckgo-slide-poll.showcase'
		);
	...
	render() {
		return (
			<Fragment>
				<AppTemplatesFixed
					selectTemplate={this.selectTemplate}></AppTemplatesFixed>
	...
	private renderPoll() {
		return (
			<div
				...
				custom-tappable
				onClick={() =>
					this.selectedTemplate.emit({ template: SlideTemplate.POLL })
				}>
				<deckgo-slide-poll
					class='showcase'
					poll-link={
						EnvironmentConfigService.getInstance().get<EnvironmentDeckDeckGoConfig>(
							'deckdeckgo'
						).pollUrl
					}
					count-answers={3}
	...
	private renderChart() {
		return (
			<div
				class='item'
				custom-tappable
				onClick={() => this.selectCharts.emit()}>
				<deckgo-slide-chart
					class='showcase'
					type='line'
	...
	private renderShapes() {
		return (
			<div
				class='item'
				custom-tappable
				onClick={() => this.addSlideAspectRatio.emit()}>
				<deckgo-slide-aspect-ratio class='showcase' grid={false}>
					<deckgo-lazy-img
						svg-src={`assets/img/shapes/robot-solid.svg`}
						aria-label={i18n.state.shapes.robot}
						class='robot'></deckgo-lazy-img>
					<deckgo-lazy-img
						svg-src={`assets/img/shapes/desktop-solid.svg`}
						aria-label={i18n.state.shapes.desktop}
						class='desktop'></deckgo-lazy-img>
					<deckgo-lazy-img
						svg-src={`assets/img/shapes/arrow-right-solid.svg`}
						aria-label={i18n.state.shapes.arrow}
						class='arrow-start'></deckgo-lazy-img>
					<deckgo-lazy-img
						svg-src={`assets/img/shapes/cloud-solid.svg`}
					...
					<deckgo-lazy-img
						svg-src={`assets/img/shapes/database-solid.svg`}
						aria-label={i18n.state.shapes.database}
						class='database'></deckgo-lazy-img>
					<deckgo-lazy-img
						svg-src={`assets/img/shapes/envelope-solid.svg`}
	...
	private renderGif() {
		...
		return (
			<div
				...
				<deckgo-slide-gif
					class='showcase'
					src={assetsStore.state.gif.exampleSrc}
					alt={i18n.state.editor.gif}>
					<p slot='top'>
	...
	private renderYoutube() {
		...
		return (
			<div
				...
				custom-tappable
				onClick={() =>
					this.selectedTemplate.emit({ template: SlideTemplate.YOUTUBE })
				}>
	...
	private renderAuthor() {
		...
		return (
			<div
				class='item'
				custom-tappable
				onClick={() => this.addSlideAuthor.emit()}>
				<deckgo-slide-author
					class='showcase'
					img-src={
	...
	private renderQRCode() {
		return (
			<div
				class='item'
				custom-tappable
				onClick={() => this.addSlideQRCode.emit()}>
				<deckgo-slide-qrcode
					class='showcase'
					content={
						EnvironmentConfigService.getInstance().get<EnvironmentDeckDeckGoConfig>(
