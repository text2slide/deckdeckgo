...
import {
	...
	insertImage,
	transformParagraph,
} from '../../../../utils/editor/paragraph.utils';
...
export class AppTransformParagraph implements ComponentInterface {
	...
	@State()
	private position:
		| { left: number; top: number; downward: boolean }
		| undefined = undefined;
	...
	onSelectParagraph({ detail: element }: CustomEvent<HTMLElement | undefined>) {
		...
		// top + size + margin
		const downward: boolean =
			top + 220 + 16 < (window.innerHeight || screen.height);
	...
	private async selectImage(
		action: EditAction,
		_image?: UnsplashPhoto | TenorGif | StorageFile | Waves
	) {
		...
		const modal: HTMLIonModalElement = await modalController.create({
			component:
				action === EditAction.OPEN_UNSPLASH
					? 'app-unsplash'
					: action === EditAction.OPEN_GIFS
					? 'app-gif'
					: 'app-storage-images',
		});
		...
		modal
			...
			.then(
				async ({
					data,
				}: OverlayEventDetail<UnsplashPhoto | TenorGif | StorageFile>) => {
	...
	private renderSlots() {
		...
		return (
			<app-slot-type
				...
				onSelectType={({ detail }: CustomEvent<SlotType>) =>
					this.transformSlot(detail)
				}></app-slot-type>
		);
	}
	...
	private renderSelectImages() {
		...
		return (
			<AppAssetChoice
				...
				) => await this.selectImage(action, image)}></AppAssetChoice>
		);
	}
}
