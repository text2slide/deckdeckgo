...
import { SlideScope, Deck, Template, elementIndex } from '@deckdeckgo/editor'; // SlideTemplate,
...
import {
	CreateSlidesUtils,
	InitTemplate,
} from '../../../utils/editor/create-slides.utils';
import {
	SlideAttributes,
	SlideTemplate,
	SlideUtils,
...
export class AppCreateSlide {
	...
	private async addSlideSplit(
		template: SlideTemplate,
		attributes?: SlideAttributes,
		elements?: SlotType[]
	) {
		const slide: JSX.IntrinsicElements =
			await CreateSlidesUtils.createSlideSplit(elements, attributes);
		await this.closePopover(template, slide);
	}
	...
	private async addRestrictedSlide(
		template: SlideTemplate | Template,
		scope: SlideScope = SlideScope.DEFAULT,
		elements?: SlotType[]
	) {
	...
	private async addSlideTemplate(
		template: SlideTemplate | Template,
		scope: SlideScope = SlideScope.DEFAULT,
		elements?: SlotType[]
	) {
	...
	private async closePopoverOpenTemplateOptions(
		template: SlideTemplate | Template,
		attributes?: SlideAttributes
	) {
		await (this.el.closest('ion-popover') as HTMLIonPopoverElement).dismiss({
	...
	private async selectSlideSlottedElements(slotType: SlotType | null) {
		...
		const customSlotsCount: number | undefined = (
			this.composeTemplate?.template as Template
		)?.data?.slots?.length;
		...
		if (slotType && (customSlotsCount === undefined || customSlotsCount > 1)) {
			...
			if (
				...
				this.elements.length <
					(this.composeTemplate?.template as Template)?.data?.slots?.length - 1
			) {
				this.elements = [...elements];
				return;
			}
		...
		if (this.composeTemplate.template === SlideTemplate.SPLIT) {
			await this.addSlideSplit(
				SlideTemplate.SPLIT,
				this.composeTemplate.attributes,
				elements
			);
	...
	private getSlideTemplate(): SlideTemplate | undefined {
		return this.composeTemplate.scope === undefined
			? SlideTemplate[
					(this.composeTemplate.template as SlideTemplate)?.toUpperCase()
			  ]
			: undefined;
	}
		...
		return (
			<ion-segment
				...
				onIonChange={($event: CustomEvent<SegmentChangeEventDetail>) =>
					(this.templatesCategory = $event?.detail?.value as
						| 'default'
						| 'community'
						| 'user')
				}
	...
	private renderToolbarTitle() {
		...
		return (
			<h2>
				{this.composeTemplate?.template === SlideTemplate.CHART
					? i18n.state.editor.select_chart
					: i18n.state.editor.compose_slide}
			</h2>
		);
	}
	...
	private renderToolbarAction() {
		...
		return (
			<app-close-menu slot='start' onClose={() => this.backCompose()}>
				<AppIcon
					name='arrow-back'
					ariaLabel={i18n.state.editor.back_to_slides}></AppIcon>
			</app-close-menu>
		);
	}
	...
	private renderTemplatesUser() {
		...
		return (
			<app-templates-user
				class='container ion-margin-bottom'
				onSelectedTemplate={async ($event: CustomEvent<Template>) =>
					await this.selectTemplateUser($event)
				}
				onNavigateSignIn={() =>
		...
		return (
			<app-templates-default
				...
				onSelectedTemplate={($event: CustomEvent) =>
					this.closePopoverOpenTemplateOptions(
				...
				onAddSlideAspectRatio={() =>
					this.addSlide(SlideTemplate['ASPECT-RATIO'])
				}
				onComposeTemplate={($event: CustomEvent<InitTemplate>) =>
					(this.composeTemplate = $event.detail)
				}
				onSelectCharts={() =>
					(this.composeTemplate = { template: SlideTemplate.CHART })
				}></app-templates-default>
	...
	private renderComposeSlide() {
		...
		if (slideTemplate === SlideTemplate.CONTENT) {
			...
			return (
				<app-templates-split
					vertical={this.composeTemplate.attributes !== undefined}
					{...attr}></app-templates-split>
			);
	...
	private renderSlotType() {
		...
		const skip: boolean =
			this.elements !== undefined && slideTemplate !== SlideTemplate.SPLIT;
		...
		return (
			<app-slot-type
				skip={skip}
				slotTypes={slotTypes}
				onSelectType={($event: CustomEvent<SlotType>) =>
					this.selectSlideSlottedElements($event.detail)
				}></app-slot-type>
		);
	}
	...
	private slotTypes(): SlotType[] | undefined {
		if (
			!this.composeTemplate ||
			!this.composeTemplate.scope ||
			this.composeTemplate.scope === SlideScope.DEFAULT
		) {
			return undefined;
		}

		return (this.composeTemplate.template as Template).data.slots?.[
