...
export class AppDocEditor implements ComponentInterface {
	...
	private readonly docUndoRedoEvents: DocUndoRedoEvents =
		new DocUndoRedoEvents();
	...
	private async initDoc() {
		...
		const Title = 'h1';
		const title: JSX.IntrinsicElements = (
			<Title key={uuid()}>{firefox ? '\u200B' : undefined}</Title>
		);
	...
	private async fetchDoc(docId: string) {
		const paragraphs: JSX.IntrinsicElements[] =
			await this.paragraphHelper.loadDocAndRetrieveParagraphs(docId);
	...
	private initEditable() {
		const onRender = (
			...
			const elements: NodeListOf<HTMLElement> =
				this.containerRef.querySelectorAll<HTMLElement>(
			...
			<Fragment>
				<ion-content class={`ion-no-padding`}>
					<main>
						...
						<AppActionsDocEditor
							containerRef={this.containerRef}></AppActionsDocEditor>
					</main>
				</ion-content>
