...
import { convertStyle, elementIndex } from '@deckdeckgo/editor'; //  SlideTemplate,
...
import {
	ExecCommandAction,
	ExecCommandStyle,
} from '../../lib/inline-editor/interfaces/interfaces';
import { execCommand } from '../../lib/inline-editor/utils/execcommand.utils';
import { DeckdeckgoInlineEditorUtils } from '../../lib/inline-editor/utils/utils';
import {
	getUserId,
	getUserEmail,
	getUserLoginType,
...
import { SlideTemplate } from '../../utils/editor/slide.utils';
...
export class AppDeckEditor implements ComponentInterface {
	...
	private containers: string =
		'h1,h2,h3,h4,h5,h6,p,section,deckgo-reveal,deckgo-reveal-list,ol,ul';
	@Element() el: HTMLElement;

	@State()
	private inlineFontFamily: string;
	...
	@Event()
	private styleDidChange: EventEmitter<HTMLElement>;

	@Event()
	private slideConcat: EventEmitter<{
		slideId: string;
	...
	async init() {
		...
		await this.deckEditorEvents.init({
			mainRef: this.mainRef,
			actionsEditorRef: this.actionsEditorRef,
		});
	...
	private resetDOM() {
		const background: HTMLElement = this.deckRef?.querySelector(
			":scope > [slot='background']"
		);
		background?.parentElement?.removeChild(background);

		const header: HTMLElement = this.deckRef?.querySelector(
			":scope > [slot='header']"
		);
		header?.parentElement?.removeChild(header);

		const footer: HTMLElement = this.deckRef?.querySelector(
			":scope > [slot='footer']"
		);
	...
	private async updateInlineEditorListener(): Promise<void> {
		...
		const inlineEditor: HTMLDeckgoInlineEditorElement = this.el.querySelector(
	...
	private async fetchSlides(deckId: string) {
		const slides: JSX.IntrinsicElements[] =
			await this.editorHelper.loadDeckAndRetrieveSlides(deckId);
	...
	private async initDeckStyle() {
		...
		if (editorStore.state.deck?.data?.attributes?.directionMobile) {
			this.directionMobile =
				editorStore.state.deck.data.attributes.directionMobile;
		}

		this.background = await ParseDeckSlotsUtils.convert(
			editorStore.state.deck.data.background,
			'background'
		);
		this.header = await ParseDeckSlotsUtils.convert(
			editorStore.state.deck.data.header,
			'header'
		);
		this.footer = await ParseDeckSlotsUtils.convert(
			editorStore.state.deck.data.footer,
			'footer'
		);

		const google: EnvironmentGoogleConfig =
			EnvironmentConfigService.getInstance().get('google');
	...
	private async concatSlide(extraSlide: JSX.IntrinsicElements) {
		/* *NOTE* get slide index */
		const slideIndex: number = this.activeIndex + 1;
		this.slides = [
			...this.slides.slice(0, slideIndex),
			extraSlide,
			...this.slides.slice(slideIndex),
		];
	...
	private async concatSlideByIndex(
		extraSlide: JSX.IntrinsicElements,
		slideIndex: number
	) {
		this.slides = [
	...
	private async replaceSlide(slide: JSX.IntrinsicElements) {
		this.slides = [
			...this.slides.map((filteredSlide: JSX.IntrinsicElements, i) =>
				i === this.activeIndex ? slide : filteredSlide
			),
		];
	}
	...
	private async copySlide($event: CustomEvent<HTMLElement>) {
		...
		const slide: JSX.IntrinsicElements = await this.editorHelper.copySlide(
			$event.detail
		);
	...
	async deleteSlide({ detail: deletedSlide }: CustomEvent<HTMLElement>) {
		...
		this.slides = [
			...this.slides.filter(
				(_slide: JSX.IntrinsicElements, index: number) => slideIndex !== index
			),
		];
	...
	@Listen('slidesDelete', { target: 'document' })
	async deleteSlides({ detail }: CustomEvent<HTMLElement[]>) {
		const slides = Array.from(
			this.el.querySelectorAll('.deck deckgo-deck [slide_id]')
		);

		detail.reverse().forEach(async (deletedSlide) => {
			const slideIndex: number = slides.indexOf(deletedSlide);
			// elementIndex(deletedSlide);
	...
	@Listen('replaceSlide', { target: 'document' })
	async addSlideToReplace($event: CustomEvent) {
		if (!$event) {
			return;
		}

		const { slideId, extraSlide, slideIndex } = $event.detail;
		await this.concatSlideByIndex(extraSlide, slideIndex);

		this.slideConcat.emit({ slideId, extraSlide });
	}
	...
	private onElementFocus($event: CustomEvent<HTMLElement>): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			const selectedElementSlideIndex: number = Array.prototype.indexOf.call(
	...
	private deckTouched($event: MouseEvent | TouchEvent): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			if (
				...
				($event.target as HTMLElement).nodeName.toLowerCase() === 'deckgo-deck'
			) {
				resolve();
				return;
			}
	...
	private async selectDeck($event: MouseEvent | TouchEvent) {
		...
		if (
			!this.contentRef.isEqualNode(src) &&
	...
	private async openFullscreenInfo() {
		const infoDisplayedOnce: boolean = await get<boolean>(
			'deckdeckgo_display_fullscreen_info'
		);
	...
	private initMainSize() {
		if (
			!this.contentRef ||
			isFullscreen() ||
			(isMobile() && !isIPad() && !isAndroidTablet())
		) {
			this.mainSize = {
				width: isMobile() ? 'calc(100% - 32px)' : '100%',
				height: isMobile() ? 'calc(100% - 32px)' : '100%',
			};
			this.fontSize = (this.mainRef.clientWidth + 64) / (720 / 48);
		...
		const width: number =
			this.contentRef.offsetWidth - (wideScreen.matches ? 164 : 32);
	...
	@Listen('slideDelete', { target: 'document' })
	@Listen('slidesDelete', { target: 'document' })
	@Listen('didReorder', { target: 'document' })
	async updatePageNumber() {
		setTimeout(async () => {
			await HeaderFooterUtils.reloadPageNumber();
		}, 500);
	}
	...
	private async updatePresenting(presenting: boolean) {
		...
		await this.remoteEvents.updateRemoteReveal(
			this.fullscreen && this.presenting
		);
	...
	private async onSlideChange() {
		await this.deckDataEvents.toggleSlideEditable(
			!this.fullscreen || !this.presenting
		);
	...
	private async onExecFontFamilyCommand(
		$event: CustomEvent<ExecCommandAction>
	) {
		if (!$event || !$event.detail) {
			return;
		}
		this.onAttributesChangesInitStyle();
		this.inlineFontFamily = ($event.detail.detail as ExecCommandStyle).value;
		await execCommand(this.selection, $event.detail, this.containers);

		const container: HTMLElement =
			await DeckdeckgoInlineEditorUtils.findContainer(
				this.containers,
	...
	private onAttributesChangesInitStyle() {
		...
		const observer = new MutationObserver(() => {
			observer.disconnect();
			this.initStyleForInlineEditor(anchorNode);
		});
		observer.observe(anchorNode, { attributes: true });
	}

	private initStyleForInlineEditor(anchorNode: HTMLElement) {
		this.findStyle(anchorNode);
	}

	private findStyle(node: HTMLElement) {
		if (!node) {
			return;
		}
		// Just in case
		...
		this.inlineFontFamily = DeckdeckgoInlineEditorUtils.getFontFamily(node);
	}

	@Listen('selectionchange', { target: 'document' })
	async onSelectionChange(_$event: UIEvent) {
		const selection: Selection = await this.getSelection();
		this.selection = selection;
		if (selection && selection.anchorNode) {
			this.initStyleForInlineEditor(selection.anchorNode.parentElement);
		}
	}
	...
	render() {
		const autoSlide: boolean =
			editorStore.state.deck?.data?.attributes?.autoSlide !== undefined
				? editorStore.state.deck.data.attributes.autoSlide
				: false;
			...
			<ion-content
				...
				<div class='editor'>
					...
					<div class='grid'>
						<div
							class='deck'
							ref={(el) => (this.contentRef = el as HTMLElement)}>
							<main
								ref={(el) => (this.mainRef = el as HTMLElement)}
								class={
									busyStore.state.slideReady
										? this.presenting
											? 'ready idle'
											: 'ready'
										: undefined
								...
								style={{
									'--main-size-width': this.mainSize?.width,
									'--main-size-height': this.mainSize?.height,
								}}>
								{this.renderLoading()}
								<deckgo-deck
									...
									autoSlide={
										this.fullscreen && this.presenting && autoSlide
											? 'true'
											: 'false'
									}
						...
						<app-breadcrumbs
							...
							onStepTo={($event: CustomEvent<HTMLElement>) =>
								this.selectStep($event)
							}></app-breadcrumbs>

						<app-actions-deck-editor
							ref={(el) =>
								(this.actionsEditorRef = el as HTMLAppActionsDeckEditorElement)
							}
							hideActions={this.hideActions}
							fullscreen={this.fullscreen}
							slides={this.slides}
							slideNumber={this.activeIndex}
							onAnimatePrevNextSlide={($event: CustomEvent<boolean>) =>
								this.animatePrevNextSlide($event)
							}
							onSlideCopy={($event: CustomEvent<HTMLElement>) =>
								this.copySlide($event)
							}
							onSlideTransform={($event: CustomEvent<JSX.IntrinsicElements>) =>
								this.transformSlide($event)
							}
							onElementFocus={($event: CustomEvent<HTMLElement>) =>
								this.onElementFocus($event)
							}
							onPresenting={($event: CustomEvent<boolean>) =>
								this.updatePresenting($event?.detail)
							}></app-actions-deck-editor>
					</div>
				</div>
	...
	private renderLaserPointer() {
		return this.fullscreen && this.presenting ? (
			<deckgo-laser-pointer></deckgo-laser-pointer>
		) : undefined;
	}
		...
		return (
			<deckgo-inline-editor
				...
				onToolbarActivated={($event: CustomEvent<boolean>) =>
					this.inlineEditorActivated($event)
				}
				...
				align={true}
				fontSize={true}
				customActions='font-family'>
				<deckgo-ie-font-family-actions
					slot='font-family'
					fontFamily={this.inlineFontFamily}
					onExecCommand={($event: CustomEvent<ExecCommandAction>) =>
						this.onExecFontFamilyCommand($event)
					}></deckgo-ie-font-family-actions>
			</deckgo-inline-editor>
		);
	}
