...
export class ImageHelper {
	...
	async imageAction(
		...
		if (imageAction.action === EditAction.OPEN_UNSPLASH) {
			...
			imageAction.action === EditAction.ADD_IMAGE &&
			imageAction.image
		) {
		...
		modal.onDidDismiss().then(async (detail: OverlayEventDetail) => {
			if (detail && detail.data && selectedElement) {
				if (action === EditAction.OPEN_CUSTOM_LOGO) {
					await this.updateSlideAttribute(
						selectedElement,
						detail.data,
						'img-src'
					);
	...
	async openCustomModalRestricted(
		selectedElement: HTMLElement,
		slide: boolean,
		deck: boolean,
		componentTag: string,
	...
	private appendImage(
		...
		return new Promise<void>(async (resolve) => {
			...
			if (slide || deck) {
				await this.appendBackgroundImg(
					selectedElement,
					image as UnsplashPhoto | TenorGif | StorageFile,
					deck
				);
			} else {
				await this.appendContentImg(
					selectedElement,
					image as UnsplashPhoto | TenorGif | StorageFile
				);
			}
	...
	private deleteBackground(
		...
		return new Promise<void>(async (resolve) => {
			...
			const currentSlotElement: HTMLElement = selectedElement.querySelector(
			...
			if (currentSlotElement) {
				...
				if (deck) {
					...
					if (selectedElement.parentElement) {
						await (
							selectedElement.parentElement as HTMLDeckgoDeckElement
						).loadBackground();
					}
				}
			}
	...
	private appendContentImg(
		selectedElement: HTMLElement,
		image: UnsplashPhoto | TenorGif | StorageFile
	): Promise<void> {
		return new Promise<void>((resolve) => {
			let element: HTMLDeckgoLazyImgElement = SlotUtils.isNodeReveal(
				selectedElement
			)
	...
	private async appendChildImg(
		...
		const img: HTMLElement = initDeckgoLazyImgAttributes({
			element: deckgoImg,
			image,
			background: true,
		});
