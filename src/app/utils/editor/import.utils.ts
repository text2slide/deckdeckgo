...
import { Deck, Doc, Paragraph } from '@deckdeckgo/editor';
...
export const importEditorData = async ({
	id,
	deck,
	slides,
	doc,
	...
	if (deck) {
		await setMany(
			slides?.map((slide: Slide) => [`/decks/${id}/slides/${slide.id}`, slide])
		);
	...
	//     `/docs/${id}/paragraphs/${paragraph.id}`,
	//     paragraph,
	//   ])
	// );
...
export const importEditorSync = async ({
	deck,
	slides,
	doc,
	paragraphs,
