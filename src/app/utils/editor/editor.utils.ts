...
const migrateDeckId = async (): Promise<Editor | undefined> => {
	const deckId: string | undefined = await get('deckdeckgo_editor');
...
export const setEditDeckId = (id: string): Promise<void> =>
	set('deckdeckgo_editor', { id, type: 'deck' });

export const setEditDocId = (id: string): Promise<void> =>
	set('deckdeckgo_editor', { id, type: 'doc' });
