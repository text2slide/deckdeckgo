...
export class SelectedElementUtils {
	...
	private static isElementText(element: HTMLElement): boolean {
		...
		return element.hasAttribute('text');
	}

	private static isElementShape(element: HTMLElement): boolean {
		if (
			!element ||
			!element.nodeName ||
			(element.nodeName.toLowerCase() !== SlotType.DRAG_RESIZE_ROTATE &&
		...
		return element.hasAttribute('shape');
	}
	...
	static initDescription(element: HTMLElement): Partial<SelectedElement> {
		...
		if (type === 'element') {
			return {
				type,
				slot: {
					math: this.isElementMath(
						SlotUtils.isNodeReveal(element)
							? (element.firstElementChild as HTMLElement)
							: element
					),
					wordCloud: this.isElementWordCloud(
						SlotUtils.isNodeReveal(element)
							? (element.firstElementChild as HTMLElement)
							: element
					),
					markdown: this.isElementMarkdown(
						SlotUtils.isNodeReveal(element)
							? (element.firstElementChild as HTMLElement)
							: element
					),
					code: this.isElementCode(
						SlotUtils.isNodeReveal(element)
							? (element.firstElementChild as HTMLElement)
							: element
					),
					image: this.isElementImage(
						SlotUtils.isNodeReveal(element)
							? (element.firstElementChild as HTMLElement)
							: element
					),
					text: this.isElementText(element),
		...
		return {
			...
			slide: {
				...
				demo:
					nodeName === 'deckgo-slide-split' &&
					element?.getAttribute('type') === SlideSplitType.DEMO,
				qrCode: nodeName === 'deckgo-slide-qrcode',
				chart: nodeName === 'deckgo-slide-chart-custom',
