...
export const getDeckBackgroundImage = async (): Promise<
  UserAsset | undefined
> => {
  return getDeckImage();
};

export const getSlidesLocalImages = async ({
  deck,
}: {
  deck: Deck | undefined;
}): Promise<UserAsset[]> => {
...
export const getSlidesOnlineImages = async ({
  deck,
}: {
  deck: Deck | undefined;
}): Promise<UserAsset[]> => {
  return getAssets<UserAsset>({
    elementIds: deck?.data?.slides,
    assets: getSlideOnlineImages,
    selector: slideSelector,
  });
};

export const getSlidesLocalCharts = async ({
  deck,
}: {
  deck: Deck | undefined;
}): Promise<UserAsset[]> => {
  return getAssets<UserAsset>({
    elementIds: deck?.data?.slides,
    assets: getSlideLocalCharts,
    selector: slideSelector,
  });
};

export const getSlidesOnlineCharts = async ({
  deck,
}: {
  deck: Deck | undefined;
}): Promise<UserAsset[]> => {
  return getAssets<UserAsset>({
    elementIds: deck?.data?.slides,
    assets: getSlideOnlineCharts,
    selector: slideSelector,
  });
};

export const getParagraphsLocalImages = async ({
  doc,
}: {
  doc: Doc | undefined;
}): Promise<UserAsset[]> => {
...
export const getParagraphsOnlineImages = async ({
  doc,
}: {
  doc: Doc | undefined;
}): Promise<UserAsset[]> => {
...
const getAssets = async <T>({
  ...
  try {
    const data: (T[] | undefined)[] = await Promise.all(
      elementIds.map((id: string) => assets(selector(id)))
    );
...
const getDeckImage = async (): Promise<UserAsset | undefined> => {
  const backgroundElement: HTMLElement = document.querySelector(
  ...
  const img: HTMLDeckgoLazyImgElement = backgroundElement.querySelector(
    SlotType.IMG
  );
...
const getLocalImages = async ({
  ...
  const list: HTMLDeckgoLazyImgElement[] = Array.from(imgs).filter(
    (img: HTMLDeckgoLazyImgElement) => {
      return isLocalImage(img);
    }
  );

  const files: File[] = await getMany<File>(
    list.map(({ imgSrc }: HTMLDeckgoLazyImgElement) => imgSrc)
  );
...
const getSlideOnlineImages = async ({
  selector,
}: {
  selector: string;
}): Promise<UserAsset[] | undefined> => {
  ...
  const promises: Promise<UserAsset | undefined>[] = list.map(
    ({ imgSrc }: HTMLDeckgoLazyImgElement) =>
      getUserAsset({ url: imgSrc, type: 'images' })
  );

  return (await Promise.all(promises)).filter(
    (asset: UserAsset | undefined) => asset !== undefined
  );
};
...
const getImages = ({
  ...
  selector: string;
}): HTMLDeckgoLazyImgElement[] | undefined => {
  ...
  const imgs: NodeListOf<HTMLDeckgoLazyImgElement> = element.querySelectorAll(
    SlotType.IMG
  );
  ...
  return Array.from(imgs).filter((img: HTMLDeckgoLazyImgElement) => {
    return !(
      ...
      !element.hasAttribute('custom-background')
    );
  });
};

const isLocalImage = ({ imgSrc }: HTMLDeckgoLazyImgElement): boolean =>
  imgSrc !== undefined && imgSrc !== '' && !imgSrc.startsWith('http');

const getSlideLocalCharts = async ({
  selector,
}: {
  selector: string;
}): Promise<UserAsset[] | undefined> => {
...
const getSlideOnlineCharts = async ({
  selector,
}: {
  selector: string;
}): Promise<UserAsset[] | undefined> => {
...
const getChartSrc = ({
  ...
  if (
    slideElement.tagName &&
    slideElement.tagName.toUpperCase() !==
      'deckgo-slide-chart-custom'.toUpperCase()
  ) {
    return undefined;
  }
