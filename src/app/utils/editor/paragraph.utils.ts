...
export const findParagraph = ({
	...
	// Just in case
	if (
		container.nodeName.toUpperCase() === 'HTML' ||
		container.nodeName.toUpperCase() === 'BODY'
	) {
		return undefined;
	}
...
export const transformParagraph = ({
	...
	const anchor: HTMLElement =
		NodeUtils.toHTMLElement(paragraph.previousSibling) || container;

	const addObserver: MutationObserver = new MutationObserver(
		(mutations: MutationRecord[]) => {
			addObserver.disconnect();

			const mutation: MutationRecord | undefined = mutations.find(
				({ addedNodes }: MutationRecord) =>
					addedNodes[0]?.nodeName.toLowerCase() === slotType
			);
