...
import { convertStyle } from '@deckdeckgo/editor'; // SlideTemplate,

import { getAttributes } from './attributes.utils';
import { InitTemplate } from './create-slides.utils';
import { SlideTemplate } from './slide.utils';
...
export class CloneSlideUtils {
	static async toggleTemplate(
		...
		const SlideElement: string = `deckgo-slide-${(
			initTemplate.template as SlideTemplate
		).toLowerCase()}`;
		...
		if (
			initTemplate.template === SlideTemplate.SPLIT &&
			!initTemplate.attributes
		) {
		...
		if (
			![]
				.concat([SlideTemplate.CONTENT, SlideTemplate.TITLE])
				.includes(initTemplate.template)
		) {
			this.toggleSlotName(selectedElement, 'title', 'start');
			this.toggleSlotName(selectedElement, 'content', 'end');
		} else if (![].concat[SlideTemplate.SPLIT].includes(initTemplate.template)) {
		...
		return (
			<SlideElement
				key={uuid()}
				{...attributes}
				innerHTML={selectedElement.innerHTML}></SlideElement>
		);
	}

	private static toggleSlotName(
		selectedElement: HTMLElement,
		currentSlot: string,
		newSlot: string
	) {
		const slot: HTMLElement | null = selectedElement.querySelector(
			`:scope > [slot="${currentSlot}"]`
		);
