...
import { SlideAttributes, SlideUtils, SlideTemplate } from './slide.utils';
...
export class CreateSlidesUtils {
	static createSlide(
		...
		return this.createSlideDefault(template, deck, createAfterIndex);
	}
		...
		return new Promise<JSX.IntrinsicElements>(async (resolve) => {
			...
			const slide: JSX.IntrinsicElements = (
				<deckgo-slide-title key={uuid()}>
					...
					{elements.length >= 2
						? createElement({ slotType: elements[1], slotName: 'content' })
						: undefined}
				</deckgo-slide-title>
			);
	...
	private static createSlideContent(
		elements: SlotType[],
		style?: { [key: string]: string }
	): Promise<JSX.IntrinsicElements | undefined> {
	...
	private static createSlideAuthor(): Promise<JSX.IntrinsicElements> {
		return new Promise<JSX.IntrinsicElements>(async (resolve) => {
          ...
          {bio !== undefined ? <div><small>{bio}</small></div> : undefined}
			...
			const imgSrc: string =
				user && user.data && user.data.photo_url
					? user.data.photo_url
					: undefined;
			const imgAlt: string =
				user && user.data && user.data.name ? user.data.name : 'Author';
	...
	static createSlidePlayground(
		src: string = undefined,
		theme: DeckdeckgoPlaygroundTheme = undefined
	): Promise<JSX.IntrinsicElements> {
		return new Promise<JSX.IntrinsicElements>((resolve) => {
			const title = <h1 slot='title'></h1>;
	...
	static createSlideChart(
		attributes: SlideAttributes = undefined
	): Promise<JSX.IntrinsicElements> {
		return new Promise<JSX.IntrinsicElements>((resolve) => {
			const title = <h1 slot='title'></h1>;
			...
			// prettier-ignore
			// @ts-ignore
			const slide: JSX.IntrinsicElements = (<deckgo-slide-chart-custom key={uuid()} {...attributes} custom-loader={true}>
          {title}
	...
	static createSlidePoll(
		...
		return new Promise<JSX.IntrinsicElements>((resolve) => {
			...
			const slide: JSX.IntrinsicElements = (
				<deckgo-slide-poll
					key={uuid()}
					pollLink={deckDeckGoConfig.pollUrl}
					socketUrl={deckDeckGoConfig.socketUrl}>
					{questionSlot}
					{...answerSlots}
		...
		return new Promise<JSX.IntrinsicElements>((resolve) => {
			const slide: JSX.IntrinsicElements = (
				<deckgo-slide-aspect-ratio
					key={uuid()}
					grid={false}
					editable={true}></deckgo-slide-aspect-ratio>
			);
				...
				{!elements || elements.length <= 0
					...
					: elements.map((element: SlotType, i: number) => {
							return createElement({
								slotType: element,
								slotName: template.data.slots?.[i]?.name,
							});
					  })}
			</Element>
		);
