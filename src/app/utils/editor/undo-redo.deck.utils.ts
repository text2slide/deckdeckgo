...
export const setAttribute = (
  target: HTMLElement,
  { attribute, value, updateUI }: UndoRedoDeckChangeAttribute
) => {
...
export const undoRedo = async ({
  ...
  if (undoRedoStore.state.elementInnerHTML !== undefined) {
    if (from) {
      const undoChange: UndoRedoChange = from[from.length - 1];
...
const undoRedoElement = (
  target: HTMLElement,
  { innerHTML }: UndoRedoDeckInputElement
) => {
...
const undoRedoSetStyle = async (
  target: HTMLElement,
  { value, type, updateUI }: UndoRedoDeckChangeStyle
) => {
...
const emitDidUpdate = ({
  ...
  target: HTMLElement;
  eventName: 'deckDidChange' | 'slideDidChange';
}) => {
  const didUpdate: CustomEvent<HTMLElement> = new CustomEvent<HTMLElement>(
    eventName,
    {
      bubbles: true,
      detail: target,
