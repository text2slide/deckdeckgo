...
export class ContrastUtils {
	static async calculateContrastRatio(
		bgColor: string | undefined,
		color: string | undefined
	): Promise<number> {
		const bgColorWithDefault: string =
			bgColor === undefined || bgColor === '' ? `rgb(255, 255, 255)` : bgColor;
		const colorWithDefault: string =
			color === undefined || color === '' || color === 'initial'
				? `rgb(0, 0, 0)`
				: color;
		...
		if (!bgRgba || bgRgba.length < 4 || bgRgba[3] >= 1) {
			return this.calculateContrastRatioOpaque(
				bgColorWithDefault,
				colorWithDefault
			);
		}
	...
	private static calculateColorContrastRatio(
		firstColorLum: number,
		secondColorLum: number
	): number {
	...
	private static async convertAlphaRgba(
		color: string,
		base: number[]
	): Promise<string> {
	...
	private static async calculateContrastRatioAlpha(
		bgColor: string,
		color: string
	): Promise<number> {
		const lumColor: number = this.calculateLuminance(extractRgb(color));

		const onBlack: { luminanceOverlay: number; ratio: number } =
			await this.calculateColorContrastRatioWithBase(
				bgColor,
				lumColor,
				[0, 0, 0, 1]
			);
		const onWhite: { luminanceOverlay: number; ratio: number } =
			await this.calculateColorContrastRatioWithBase(
				bgColor,
				lumColor,
				[255, 255, 255, 1]
	...
	private static async calculateContrastRatioOpaque(
		bgColor: string,
		color: string
	): Promise<number> {
