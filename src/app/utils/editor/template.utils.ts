import { Template, SlideScope, injectJS } from '@deckdeckgo/editor';

import templatesStore from '../../stores/templates.store';
import { Slide } from './slide.utils';
...
export class TemplateUtils {
	...
	static async loadSlideTemplate(slide: Slide) {
		const template: Template | undefined = await this.getTemplate(
			slide.data.scope,
			slide.data.template
		);
	...
	static async getTemplate(
		...
		const templates: Template[] =
			scope === SlideScope.COMMUNITY
				? templatesStore.state.community
				: templatesStore.state.user;

		return templates.find(
			(filteredTemplate: Template) => filteredTemplate.data.tag === template
		);
	}
}
