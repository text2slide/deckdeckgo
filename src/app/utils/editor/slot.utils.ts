...
export class SlotUtils {
	...
	static isSlotTypeEditable(type: SlotType): boolean {
		return (
			type !== SlotType.IMG && type !== SlotType.SOCIAL && type !== SlotType.DEMO
		);
	}

	static isNodeDragDropResize(selectedElement: HTMLElement | null): boolean {
		return (
			selectedElement?.nodeName?.toLowerCase() === SlotType.DRAG_RESIZE_ROTATE ||
			selectedElement?.nodeName?.toLowerCase() === SlotType.DRAG_RESIZE_ROTATE_TEXT
		);
	}
	...
	static isNodeTitle(selectedElement: HTMLElement): boolean {
		...
		].includes(selectedElement.nodeName.toLowerCase());
	}
}
