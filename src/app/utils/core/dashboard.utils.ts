...
import { Slide } from '../editor/slide.utils';
import { EnvironmentDeckDeckGoConfig } from '../../types/core/environment-config';
import { EnvironmentConfigService } from '../../services/environment/environment-config.service';

export type DeckOrDoc =
	| { deck: Deck; doc?: never }
	| { doc: Doc; deck?: never };
...
const cloneDeck = async ({
	deck,
}: DeckOrDoc): Promise<ImportData | undefined> => {
	if (!deck) {
		return undefined;
	}
	...
	const promises: Promise<Slide>[] | undefined = deck.data.slides?.map(
		(slideId: string) => getSlide(deck.id, slideId)
	);
	...
	// /* *NOTE* API /copyData */
	// // copyData's data: deckId
	// const copyDataApiData = {
	// 	deckId: deck.id,
	// };
	// const copyDataRt = await fetchData(ApiUrls.copyData, copyDataApiData);
	// const copiedDeck: Deck = copyDataRt.decksModel;
	// const copiedSlides: Slide[] = copyDataRt.slideModels;
...
// 	const promises: Promise<Paragraph>[] | undefined = doc.data.paragraphs?.map(
// 		(paragraphId: string) => getParagraph({ docId: doc.fileId, paragraphId })
// 	);
// 	const paragraphs: Paragraph[] = await Promise.all(promises || []);

// 	const cloneParagraphs: Paragraph[] = paragraphs.map(
// 		(paragraph: Paragraph) => ({
// 			...paragraph,
// 			id: uuid(),
// 		})
...
export const loadAndImportDoc = (doc: Doc): Promise<void> => {
	return new Promise<void>(async (resolve, reject) => {
		try {
			const promises: Promise<Paragraph>[] | undefined =
				doc.data.paragraphs?.map((paragraphId: string) =>
					getParagraph({
						docId: doc.id,
						paragraphId,
					})
...
export const navigateReloadEditor = () => {
	...
	navStore.state.nav = {
		url: config.navigateReloadEditorUrl,
...
export const deleteDeckOrDoc = async (data: DeckOrDoc) => {
	if (data.doc) {
		...
		await Promise.all([
			deleteStorageFile({ meta: doc.data.meta, folder: 'd' }),
			deleteDoc(doc.id),
		]);
		return;
	}
	...
	await Promise.all([
		deleteStorageFile({ meta: deck.data.meta, folder: 'p' }),
		deleteDeck(deck.id),
	]);
};
