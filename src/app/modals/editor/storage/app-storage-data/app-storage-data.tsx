	...
	private upload(): Promise<void> {
		return new Promise<void>(async (resolve) => {
			...
			if (filePicker.files && filePicker.files.length > 0) {
				...
				// /* *NOTE* API /saveUploadData: csv */
				// // file to upload
				// const pickedFile = filePicker.files[0];

				// // format data as FormData
				// const saveUploadDataData = new FormData();
				// saveUploadDataData.append('uploadDocs', pickedFile, pickedFile.name);

				// await saveUploadDataApi(ApiUrls.saveUploadData, saveUploadDataData);

				// save to local
				const storageFile: StorageFile =
					await this.storageOfflineProvider.uploadFile(
						filePicker.files[0],
						'data',
						5242880
					);
	...
	render() {
		return [
			...
			<ion-content class='ion-padding'>
				<app-storage-files
					folder={'data'}
					onSelectAsset={async ($event: CustomEvent<StorageFile>) =>
						await this.selectData($event.detail)
					}></app-storage-files>
