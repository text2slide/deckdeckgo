...
export class AppTemplate {
	...
	private async onFileInput() {
		...
		if (this.inputFileRef.files && this.inputFileRef.files.length > 0) {
			...
			if (components && components.length > 0) {
				const cmp: TemplateData | undefined = components.find(
					(cmp: TemplateData) => cmp.tag === this.templateData.tag
				);
				this.templateData = {
					...this.templateData,
	...
	private async validateCDNInput() {
		try {
			...
			this.validCdn =
				/unpkg\.com|cdnjs\.cloudflare\.com|cdn\.jsdelivr\.net/g.test(
					url.hostname
				);
		} catch (err) {
			this.validCdn = false;
	...
	render() {
		const errorCdn: string | undefined =
			!this.validCdn &&
			this.templateData?.cdn !== '' &&
			this.templateData?.cdn !== undefined
				? 'error'
				: '';
		const errorTag: string | undefined =
			!this.validTag &&
			this.templateData?.tag !== '' &&
			this.templateData?.tag !== undefined
				? 'error'
				: '';
					...
					<main class='ion-padding'>
						<form onSubmit={(e: Event) => this.handleSubmit(e)}>
							...
							<ion-button
								...
								shape='round'
								disabled={!this.validCdn || !this.validTag}>
			...
			<div class='notes'>
				<p class={`small`}>
					...
					{renderI18n(i18n.state.templates.url, {
						...
						value: (
							<mark>
								https://unpkg.com/my-component@latest/dist/my-component/my-component.esm.js
							</mark>
						),
					})}
				</p>
