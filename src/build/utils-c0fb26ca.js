...
class DeckdeckgoInlineEditorUtils {
  ...
  static async getFontFamily(element) {
    if (element.style.fontFamily)
      return await this.getFontFamilyName(element.style.fontFamily);
    if (!element.hasChildNodes())
      return undefined;
    const children = element.children;
    if (children && children.length > 0) {
      const selectedChild = Array.from(children).find(async (child) => {
        return await this.getFontFamilyName(child.style.fontFamily);
      });
      if (selectedChild) {
        return await this.getFontFamilyName(selectedChild.style.fontFamily);
      }
    }
    return undefined;
  }
  static async getFontFamilyName(font) {
    if (font) {
